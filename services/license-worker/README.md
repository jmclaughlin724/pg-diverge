...
- `PUT /contracts?repo=acme/app&name=main` stores a schema contract when the request carries an unexpired repo-bound license token in `Authorization`. The payload must match the `SchemaContract` JSON shape from `src/schema-contract.ts`; drift comparison remains in the package library through `contractDrift`.
