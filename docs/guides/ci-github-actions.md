...
- The committed composite action wraps any invocation: `uses: jmclaughlin724/supaschema@v0.1.1` with `args: "diff --fail-on-diff --quiet"` and `SUPASCHEMA_DATABASE_URL` in the step env. Always pin the action tag; by default the action runs the exact npm package version shipped with that tag, and the optional `version` override only accepts exact npm semver versions.
