...
The gate is a command set: `scan --reporter json`, `diff --fail-on-diff`, `check`, and optional `verify`. Protected deploy jobs can run `sync` with configured targets and an explicit remote approval environment variable. This repository ships CLI and GitHub Action gates, local paid-pack entitlement checks, and no hosted CI, live billing deployment, or marketplace distribution.
