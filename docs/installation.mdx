...
After install, run `<local-runner> init`. It uses the consuming repo's package manager, workspace owner, provider markers, schema paths, and migration paths to create config and directories. It does not install active AI-agent rules, hooks, skills, settings, `AGENTS.md`, or `CLAUDE.md` by default. Inspect `supaschema.config.json`, and inspect `.supaschema/install.json` only when it exists for pending path confirmation. Then run:

`<local-runner> --version` `<local-runner> config validate --json`

Use the package scripts only for routine schema changes: `supaschema:migration` generates migration SQL, `supaschema:check` validates replay safety, and `supaschema:types` refreshes generated contracts. Use direct CLI commands for setup diagnostics, database execution verification, and apply: `<local-runner> config validate --json`, `<local-runner> verify`, and `<local-runner> sync`.
