...
| pnpm | `pnpm add supaschema --config.minimumReleaseAge=0` | `pnpm exec supaschema init` | `pnpm exec supaschema <cmd>` |
...
For pnpm, `pnpm add supaschema --config.minimumReleaseAge=0` adds the dependency to the package in the current directory and lets a just-published supaschema version resolve immediately. Run `pnpm add -w supaschema --config.minimumReleaseAge=0` only when the workspace root owns the schema workflow. pnpm v11 defaults `minimumReleaseAge` to one day; keep the default only when you intentionally want pnpm to wait before accepting a fresh package version.
