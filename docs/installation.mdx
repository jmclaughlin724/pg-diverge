...
Use `supaschema:sync` for routine schema changes. It generates migration SQL, validates replay safety, refreshes generated contracts, stages generated migrations, verifies pending migrations, and applies or dry-runs through one configured target. Use `supaschema:diff`, `supaschema:stage`, `supaschema:apply`, `supaschema:types`, and `supaschema:check` only for focused lanes. Use direct CLI commands for setup diagnostics and database execution verification: `<local-runner> config validate --json` and `<local-runner> verify`.
