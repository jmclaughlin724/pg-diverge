...
keywords: ["verify command","apply twice","idempotency","catalog fingerprint"]
---
...
npx supaschema verify \
  --from "git:HEAD" --to dir:database/schemas \
...
<ParamField path="--to" type="source">
  The **after-state** (target model) source. Applied directly to the second temp database for the convergence comparison. Accepts the same specifiers as `--from`. Typically `dir:database/schemas` or your configured schema path.
</ParamField>
...
<ParamField path="--migrations-dir" type="path">
  Overrides the directory used when resolving the default `--migration` file. Defaults to `config.migrationsDir`.
</ParamField>
...
<ParamField path="--ensure-environment" type="boolean">
  Stubs the minimal Supabase-managed surfaces that common schema trees reference: `auth.users`, `auth.uid()`, `auth.role()`, `auth.jwt()`, `auth.email()`, `cron.job`, and `cron.job_run_details`. It does not create storage, realtime, vault, identities, sessions, or extension-owned objects. Enable it explicitly when a bare PostgreSQL verification server needs those surfaces.
</ParamField>

<ParamField path="--no-ensure-environment" type="boolean">
  Disables the Supabase environment stub when a wrapper or config already enabled it. Use this only when the verification server provisions managed schemas in newly-created databases.
</ParamField>
