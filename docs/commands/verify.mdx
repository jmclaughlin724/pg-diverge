...
<ParamField path="--ensure-environment" type="boolean">
  Stubs the minimal Supabase-managed surfaces that common schema trees reference: `auth.users`, `auth.uid()`, `auth.role()`, `auth.jwt()`, `auth.email()`, `cron.job`, and `cron.job_run_details`. It does not create storage, realtime, vault, identities, sessions, or extension-owned objects. Enabled automatically when the `auto` adapter is active.
</ParamField>

<ParamField path="--no-ensure-environment" type="boolean">
  Disables the Supabase environment stub even when the `auto` adapter would enable it. Use this only when the verification server provisions managed schemas in newly-created databases.
</ParamField>
