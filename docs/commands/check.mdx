...
Use `check` before any migration reaches a database.

It reads migration SQL. It does not connect to a database.

`check` looks for:

- replay-safety problems;
- lock hazards;
- parser errors;
- unsafe `DROP` and `CASCADE` patterns;
- non-idempotent data changes.

<Info>
  Run `npx supaschema check` with no arguments to check every `.sql` file in
...
<Note>
  With no arguments, supaschema scans the configured migrations directory for
  `*.sql` files. Explicit arguments must be migration files, shell-expanded
  globs, or `-` for standard input; explicit directory arguments are not
  expanded.
</Note>
...
<ParamField path="--reporter" type="text | github | sarif | json">
  Controls the diagnostic output format. Use `text` for human-readable output
  with file names, line numbers, rule codes, and messages. Use `github` to emit
  [workflow commands](https://docs.github.com/en/actions/using-workflows/workflow-commands-for-github-actions)
  that GitHub Actions renders as inline PR annotations. Use `sarif` to write a
  SARIF 2.1.0 document for GitHub code scanning. Use `json` for programmatic
  consumption or custom reporters.
</ParamField>

<ParamField path="--quiet" type="boolean">
  Suppresses all output when no errors are found. Only prints diagnostics when
  the exit code would be non-zero. Useful in pre-commit hooks where you only
  want noise on failure.
</ParamField>
...
<Warning>
  `DROP ... CASCADE` is an unconditional error. supaschema will never approve
  it, regardless of hints or configuration. Explicitly enumerate the dependent
  objects you want to drop instead.
</Warning>
...
<Tip>
  Use `CREATE INDEX CONCURRENTLY` to build indexes without blocking reads — just
  make sure the statement runs **outside** of a `BEGIN ... COMMIT` block.
  supaschema flags both patterns: non-concurrent index creation and concurrent
...
<Note>
  `check` never exits with code `1` unless the command itself fails to parse
  arguments or read files. A clean exit always means `0` (pass) or `2` (rule
  violations found).
</Note>
