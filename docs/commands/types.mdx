...
You don't have to invoke `types` manually after every schema change. The `diff` command refreshes the configured `typesFile` and `zodFile` after it successfully writes a new migration file. Use `--dry-run` or `--out stdout` when you want to preview a diff without writing a migration or refreshing generated type files.
