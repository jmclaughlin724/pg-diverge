...
`supaschema types` walks the schema source you point it at (by default the first directory listed under `schemaPaths` in your config; pass `--from` to read a different source or to combine multiple roots), builds an in-memory model of every table, view, enum, and composite type, then writes two files:
