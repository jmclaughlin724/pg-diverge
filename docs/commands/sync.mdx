...
npx supaschema sync --target staging
npx supaschema --env staging sync --target staging
npx supaschema sync --target staging --runner direct
```
...
<ParamField path="--target" type="name">
  Override config-selected targets with one configured `sync.targets.<name>`
  target.
</ParamField>
