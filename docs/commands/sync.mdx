...
3. Generate the diff unless `--no-diff` is set.
4. Select zero or one configured sync target.
5. Reconcile migration history for the selected target or fallback database URL.
6. Check pending migration files.
7. Refresh generated TypeScript/Zod contracts according to workflow policy, even when no migration is pending.
8. Stage generated migration files when Git is available.
9. Report no-op when no migration is pending.
10. Run deploy safety gates.
11. Verify the ordered pending migration set against a disposable database.
