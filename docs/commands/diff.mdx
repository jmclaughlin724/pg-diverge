...
If configured type files already exist, `diff` refreshes them after a successful migration write.
...
<Note>
  Type refresh only runs for configured files that already exist. Run `npx supaschema types` once to create them.
</Note>
...
<ParamField path="--from" type="source">
  The **before-state** source to diff against. Accepts a `database:<url>` live connection (including `database:$ENV_VAR`), a `git:<ref>` specifier (e.g. `git:HEAD`, `git:origin/main`), or a `dir:<path>` pointing to a schema directory. Defaults to `config.sources.from`; `"auto"` resolves a database URL first, then falls back to `git:HEAD`.
</ParamField>

<ParamField path="--to" type="source">
  The **after-state** source representing the desired schema. Accepts the same specifiers as `--from`, including `database:<url>` for live database sources. Defaults to `config.sources.to`, then `dir:<config.schemaPaths[0]>`.
</ParamField>
