...
`--from` and `--to` choose schema sources. The configured `migrationsDir` is a separate source-intent corpus: existing reviewed migrations are parsed for row backfills, explicit DML/`DO` workflows, enum rewrite recipes, Vault references, workload-proven index intent, and provider bootstrap constraints. If that intent is missing, `diff` should block with an actionable diagnostic instead of inventing it.
