...
<ParamField path="--to" type="source">
  Desired-state source. Defaults to `config.sources.to`, then the first configured schema path.
</ParamField>

<ParamField path="--out" type="path | stdout">
  Write SQL to a specific file or print to stdout. `stdout` skips generated output refresh.
</ParamField>
