...
2. Reads `--to` from `dir:<config.schemaPaths[0]>`.
...
<ParamField path="--to" type="source">
  The **after-state** source representing the desired schema. Accepts the same specifiers as `--from`, including `database:<url>` for live database sources. Defaults to `dir:<config.schemaPaths[0]>`.
</ParamField>
