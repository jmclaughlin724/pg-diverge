...
<Accordion title="init — Write a config file">
  `init` recreates the small `supaschema.config.json` scaffold when needed. Normal package installation already writes this file when no config exists. If a config file already exists, the command exits with an error rather than overwriting it.
  ...
  Edit the file only when you need named environments, custom schema paths, or different output paths.
</Accordion>
