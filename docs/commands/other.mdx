...
<Accordion title="plan — Print the diff plan as JSON">
  ...
  The JSON output is an object with a top-level `objects` array. Each element describes a single DDL object, the operation (`create`, `alter`, `drop`), and the ordered list of SQL statements that would be emitted.
