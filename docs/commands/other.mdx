...
<Accordion title="audit — Report schema coverage">
  ...
  The output is a table grouped by object type (tables, views, functions, triggers, policies, indexes, …) with counts and a supported/unsupported breakdown. Objects in the `unsupported` category are passed through verbatim in generated migrations rather than being diff-managed.
