...
`supaschema sync` runs deploy-blocking type-safety gates through `workflow.type_safety`; that package-owned deploy gate is separate from the standalone `type-contract --enforce` license path.
