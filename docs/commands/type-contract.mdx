...
| ------------ | ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--from`     | source                          | Previous schema source. Defaults to `git:HEAD`.                                                                                                   |
| `--to`       | source                          | New schema source. Defaults to the declarative tree.                                                                                              |
| `--reporter` | text \| json \| github \| sarif | Output format. An unknown value exits `2` with an error.                                                                                          |
| `--enforce`  | boolean                         | Fail (exit `2`) on a breaking change in the standalone command. The `sync` deploy-safety gate is configured separately by `workflow.type_safety`. |
