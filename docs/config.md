...
`pg-diverge` looks for config in this order:

1. `--config <path>` (`.json`, `.mjs`, or `.js` with a default export)
2. `pg-diverge.config.json` in the working directory
3. `pg-diverge.config.mjs`, then `pg-diverge.config.js`
4. Built-in defaults
...
| `environments` | `{}` | Named database targets for the global `--env` flag: `{ "staging": { "databaseUrl": "$STAGING_DB" } }` lets `pg-diverge --env staging diff ...` resolve the URL (with `$ENV_NAME` indirection) without repeating it per command. |
...
A configured validator that is not installed produces `PD_VALIDATOR_UNAVAILABLE` as an error: configured checks may not silently skip.
