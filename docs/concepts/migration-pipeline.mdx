...
supaschema's model is a structured pipeline of six stages — Extract, Plan, Render, Check, Verify, and Apply. `supaschema diff` performs the first three (Extract, Plan, Render) and writes the migration file; **Check, Verify, and Apply are separate, explicit steps** you run on the result (`supaschema check`, `supaschema verify`, and your deployment runner). Understanding each stage helps you interpret error messages, tune safety settings, and trust the output. This page walks through every stage in detail.

<Note>
  supaschema **never writes to your target database**. Generating a migration from `dir:` files or a `git:` ref needs no database at all; a `database:` source connects only to **read** the current catalog (it issues read queries and makes no changes — though it does not set the session `READ ONLY`). The Verify stage (below) is the one step that needs a server, and it creates and drops its own **disposable** databases — never your target. Applying the migration is always a separate, explicit step owned by your runner (such as the Supabase CLI or a custom script).
</Note>
...
<Steps>
  ...
  * Policy dependencies (tables and roles before RLS policies)
  ...
  Check is a **separate, explicit step** — `diff` writes the migration (and validates the lineage chain), but it does not run the replay-safety rules. You run them with `supaschema check` (the bundled agent hook and `supaschema sync` invoke it for you). This stage runs more than 20 replay-safety rules against the rendered SQL without executing it.
...
supaschema stops at the end of the Verify stage. It outputs the finished migration SQL to your configured migrations directory (`database/migrations` in the neutral scaffold) and then steps back.
