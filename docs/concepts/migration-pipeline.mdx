...
<Note>
  supaschema **never modifies your database**. Generating a migration from `dir:` files or a `git:` ref needs no database at all; a `database:` source opens a **read-only** catalog connection to read the current schema. Applying the migration is always a separate, explicit step owned by your runner (such as the Supabase CLI or a custom script).
</Note>
