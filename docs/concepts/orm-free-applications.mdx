...
- **Runtime validation is generated too.** `database.zod.ts` provides one direct schema-qualified validator for each generated type.
...
```ts app/accounts.ts
import type { AppAccountsInsert, AppAccountsRow } from "../database.types";
import { AppAccountsInsertSchema, AppAccountsRowSchema } from "../database.zod";

export async function createAccount(
  db: { query<T>(sql: string, values: unknown[]): Promise<{ rows: T[] }> },
  body: unknown
): Promise<AppAccountsRow> {
  const input: AppAccountsInsert = AppAccountsInsertSchema.parse(body);
  const result = await db.query<AppAccountsRow>(
    "insert into app.accounts (name) values ($1) returning id, name",
    [input.name]
  );

  return AppAccountsRowSchema.parse(result.rows[0]);
}
```

This pattern uses a driver or platform client to execute SQL. The generated named types and validators own compile-time shapes and runtime boundaries. An ORM can still be layered on top for query-builder ergonomics, but it should not duplicate schema ownership, generated types, validation contracts, or migration control.
