...
- **Runtime validation is generated too.** `database.zod.ts` provides direct Zod owners and inferred `Tables`, `TablesInsert`, `TablesUpdate`, `Enums`, and `CompositeTypes` helpers from the same model.
...
```ts app/accounts.ts
import { Tables, type TablesInsert } from "../database.zod";

type Account = Tables<{ schema: "app" }, "accounts">;
type NewAccount = TablesInsert<{ schema: "app" }, "accounts">;

export async function createAccount(
  db: { query<T>(sql: string, values: unknown[]): Promise<{ rows: T[] }> },
  body: unknown
): Promise<Account> {
  const input: NewAccount = Tables.app.accounts.Insert.parse(body);
  const result = await db.query<Account>(
    "insert into app.accounts (name) values ($1) returning id, name",
    [input.name]
  );

  return Tables.app.accounts.Row.parse(result.rows[0]);
}
```
