...
<Tip>
  Defaults come from `supaschema.config.json`. `sources.from` owns the before-state and `sources.to` owns the after-state for zero-source-flag commands. The scaffolded `"auto"` before-state resolves a database URL, then falls back to `git:HEAD`.
