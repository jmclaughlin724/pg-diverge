...
keywords:
  ["schema sources", "SQL files", "Git refs", "database URL", "snapshots"]
---
...
The configured `migrationsDir` is a third input to planning, but it is not a `--from` or `--to` schema source. Existing reviewed migrations form the source-intent corpus for row backfills, explicit DML/`DO` workflows, enum rewrite recipes, Vault references, workload-proven index intent, and provider bootstrap constraints. supaschema should model that intent before blocking; when the intent is absent, the diagnostic should tell an agent where to declare it.
...
<Tip>
  Defaults come from `supaschema.config.json`. `sources.from` owns the before-state, `sources.to` owns the after-state, and `migrationsDir` owns migration-derived source intent for zero-source-flag commands. The scaffolded `"auto"` before-state resolves valid `git:HEAD`, then a database URL, then `empty:`.
