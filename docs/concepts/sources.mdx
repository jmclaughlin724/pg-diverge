...
<Tip>
  Defaults come from `supaschema.config.json`. `sources.from` owns the before-state, `sources.to` owns the after-state, and `migrationsDir` owns migration-derived source intent for zero-source-flag commands. For generation, the scaffolded `"auto"` before-state resolves valid `git:HEAD`, then `empty:` only for a first migration with no existing migration corpus.
