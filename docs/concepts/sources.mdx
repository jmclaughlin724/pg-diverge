...
<Tip>
  For most teams, the `--from` source defaults to the live database URL (from `SUPASCHEMA_DATABASE_URL`, a named `environments` entry, or local `supabase/config.toml` discovery), while `--to` defaults to the first configured schema path. The neutral scaffold uses `dir:database/schemas`; provider-aware installs may use paths such as `dir:supabase/schemas`, `dir:neon/schemas`, `dir:aws-postgresql/schemas`, `dir:cloud-sql/schemas`, `dir:alloydb/schemas`, or `dir:azure-postgresql/schemas`. You can encode paths in `supaschema.config.json` so that every `diff` command picks them up automatically:
