...
You edit schema files. `supaschema sync` renders the migration, checks replay safety, regenerates contracts, stages generated migrations when Git is available, runs source-model safety gates, and runs the guarded apply or dry-run lane for one configured target. `diff`, `check`, `types`, `stage`, and `apply` remain available for focused operation.
