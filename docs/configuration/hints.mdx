...
For `SELECT` and `DELETE`, the rule reads `USING`. For `INSERT`, it reads `WITH CHECK`. For `UPDATE` and `ALL`, it uses `WITH CHECK` when present and otherwise applies PostgreSQL's `USING` fallback.
