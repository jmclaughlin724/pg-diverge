...
You usually do not need to repeat the URL on every command. Configure each local, staging, preview, or production PostgreSQL target once, then select it with `--env` or reference it from a named `sync.targets` entry.
...
`sync.targets` names the apply or deploy targets that `supaschema sync` can reconcile. A target must select exactly one URL owner: either `environment` or `databaseUrl`.
...
`mode: "auto"` makes a target selectable by bare `supaschema sync` when `workflow.migration_sync` is `"auto"`. The installed default is `"auto"`, so target config decides selection: local, staging, preview, and remote targets all use the same `mode` rule. Remote targets also require a runtime approval variable such as `SUPASCHEMA_REMOTE_SYNC_APPROVED=1` before automatic deploy.
