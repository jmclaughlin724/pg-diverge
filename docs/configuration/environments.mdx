...
You usually do not need to repeat the URL on every command. Configure each local, staging, preview, or production PostgreSQL target once, then select it with `--env`.
