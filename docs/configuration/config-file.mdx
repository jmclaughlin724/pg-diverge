...
| --- | --- | --- |
| Schema tree and migration corpus | `schemaPaths`, `sources.to`, `migrationsDir` | Read every nested `.sql` file under `database/schemas`, diff toward `dir:database/schemas`, write migrations to `database/migrations`, and read existing migrations there as source intent. |
...
| Generated contracts | `typesFile`, `zodFile`, `workflow.type_generation`, `workflow.zod_generation`, `workflow.type_usage` | `supaschema types` creates or refreshes TypeScript and Zod outputs, and agents use Zod validators at runtime boundaries when configured. |
...
`schemaPaths` is recursive. A root such as `database/schemas` can contain nested folders like `database/schemas/app/tables.sql` or `database/schemas/auth/policies/read.sql`; supaschema reads every nested `.sql` file and merges the parsed objects into one schema model. `sources.to` stays explicit because it is the command default for zero-source-flag `diff`, `plan`, and `verify`. `migrationsDir` is both the generated output directory and the existing-migration source-intent corpus for operational facts such as row backfills, explicit DML/`DO` workflows, enum rewrite recipes, Vault references, workload-proven index intent, and provider bootstrap constraints.
...
| `migrationsDir` | `"database/migrations"` | Where zero-flag `diff` writes migrations, zero-arg `check` reads them, `verify` finds the newest pending file, and planning reads existing migrations as source intent. Install can scaffold provider-specific folders; the canonical path table lives in [Setup](/setup#path-selection). The `--migrations-dir` flag overrides per command. |
| `typesFile` | `"database.types.ts"` | Where `supaschema types` writes generated TypeScript types. |
| `zodFile` | `"database.zod.ts"` | Where `supaschema types` writes generated runtime Zod validators (requires `zod` in the consuming project). |
...
| `schemaPaths` | `["database/schemas"]` | Paths read by `git:` sources and the default `--to dir:` target (the first entry). Install can scaffold provider-specific folders; the canonical path table lives in [Setup](/setup#path-selection). Configure the final paths here. `_bootstrap` directories under schema roots are skipped as provider/bootstrap inventory. |
