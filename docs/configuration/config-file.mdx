...
| `sync.targets.<name>` | `local` and `remote` targets | Named sync targets. Each target sets `mode`, `runner`, and `historyTable`; it may use `databaseUrl`, use `environment`, or omit both for the runner/default URL fallback. Remote targets also set `requireApprovalEnv`. |
