...
<Note>
  ...
  determine what belongs in your declarative tree, see the
...
Legacy adapter values (`postgres`, `supabase`, and `supabase-auto`) are accepted
when loading older configs and normalized to `auto`. New configs should write
`auto`.
