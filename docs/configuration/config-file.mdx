...
| --- | --- | --- |
| Schema tree and migration context | `schemaPaths`, `sources.to`, `migrationsDir` | Read every nested `.sql` file under `database/schemas`, diff toward `dir:database/schemas`, write migrations to `database/migrations`, read existing migrations as source intent, and use generated lineage there as migration-tree baseline proof. |
| Diff baseline | `sources.from`, `sources.to` | Diff from `"auto"` to the configured schema tree. For generation, `"auto"` resolves `git:HEAD` only as a candidate baseline, then must match generated migration-tree lineage when migrations exist. |
...
`schemaPaths` is recursive. A root such as `database/schemas` can contain nested folders like `database/schemas/app/tables.sql` or `database/schemas/auth/policies/read.sql`; supaschema reads every nested `.sql` file and merges the parsed objects into one schema model. `sources.to` stays explicit because it is the command default for zero-source-flag `diff`, `plan`, and `verify`. `migrationsDir` is the generated output directory, the existing-migration source-intent corpus for operational facts such as row backfills, explicit DML/`DO` workflows, enum rewrite recipes, Vault references, workload-proven index intent, and provider bootstrap constraints, and the generated-lineage baseline proof for source-backed generation.
...
| `hints.routineDependencies` | `{}` | Routine object keys mapped to explicit relation or column dependencies for dynamic SQL or unsupported routine languages, for example `{ "function:app.lookup()": ["app.accounts", "app.accounts.secret_id"] }`. |
