...
| Diff baseline | `sources.from`, `sources.to` | Diff from `"auto"` to the configured schema tree. `"auto"` resolves `git:HEAD`, then a database URL, then `empty:`. |
...
| `sources.from` | `"auto"` | Default before-state source for zero-source-flag `diff`, `plan`, and `verify`. `"auto"` resolves valid `git:HEAD`, then a database URL, then `empty:` for a first migration in a fresh repository. Pin `dump:`, `dir:`, `git:`, `database:`, `catalog:`, or `empty:` when the project has a specific baseline source. |
