...
| Diff baseline | `sources.from`, `sources.to` | Diff from `"auto"` to the configured schema tree. For generation, `"auto"` resolves `git:HEAD`, then `empty:` only for a first migration with no existing migration corpus. |
