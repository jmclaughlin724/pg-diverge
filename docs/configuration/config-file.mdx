...
Provider detection can change only the initial `schemaPaths`, `sources.to`, and `migrationsDir` values during install. For example, Supabase projects default to `supabase/schemas`, `dir:supabase/schemas`, and `supabase/migrations`; Neon projects default to `neon/schemas`, `dir:neon/schemas`, and `neon/migrations`.

The config schema is generated from `src/config.ts` into the shipped `supaschema-config.schema.json`. This page is the public reference for those fields.
...
Legacy config values `supabase-auto`, `supabase`, and `postgres` are accepted and normalized to `auto` so older projects keep loading. For backward compatibility, a legacy `adapter: "postgres"` that does not set `managedSchemas` keeps the provider-neutral posture it had before adapters were unified — managed-schema enforcement stays off, equivalent to `managedSchemas: []` — so a plain-PostgreSQL tree that owns schemas like `auth` or `storage` does not start failing after the upgrade. Set `managedSchemas` explicitly to opt into the Supabase defaults. Do not use these legacy values in new config.
