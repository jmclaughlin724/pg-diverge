...
The repository benchmark harness compares `supaschema` with Supabase CLI diff engines on identical fixtures. On the 1,000-table fixture, `supaschema` generates the migration without a database-backed shadow workflow, while database-backed workflows need multiple steps and a database that has already caught up.
