...
`supaschema` is best when you want deterministic PostgreSQL or Supabase migrations from declarative SQL files without Docker, a shadow database, or a hand-edited generated migration. The Supabase CLI remains the standard runner for applying migrations to Supabase; `supaschema` is designed to generate and prove the SQL before that apply step.

## Short answer

Use `supaschema` when the schema files are the source of truth and you need a fast, replay-safe migration from those files. Use the Supabase CLI to apply the reviewed migration with `supabase db push` after `supaschema check` and, when possible, `supaschema verify`.
...
| Applies to production | No, generation/proof only | Yes, via Supabase migration commands |
...
`diff` writes the migration and refreshes existing type outputs. `check` catches replay-safety and lock hazards. `verify` applies the migration twice in disposable databases and confirms the result matches the declared target. `supabase db push` remains the apply step.
...
The repository benchmark harness compares `supaschema` with Supabase CLI diff engines on identical fixtures. On the 1,000-table fixture with type outputs present, `supaschema` generates the migration and refreshes types in one command, while database-backed workflows need multiple steps and a database that has already caught up.
...
Keep using the Supabase CLI for:

- Applying the final migration to local, staging, or production Supabase databases.
...
Add `npx supaschema verify` before merge when CI can reach a disposable PostgreSQL database.
