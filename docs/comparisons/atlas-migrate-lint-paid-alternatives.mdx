...
Starting with Atlas v0.38 (released 2025-10-28), the `atlas migrate lint` command is available only to Atlas Pro users, and the custom schema-policy linting rules are Pro-only as well. Atlas Pro is a paid plan ($9 per seat per month at the time of writing) with a 30-day trial.

<Info>
  Last verified 2026-06-17 against the Atlas documentation. Pricing and feature
...
`supaschema` is open-core. The migration-safety checks above are free. Two analysis packs are paid: the TypeScript/Zod type-contract breaking-change gate and the grant-drift least-privilege gate. Those are not part of the `atlas migrate lint` overlap; they are additional gates a downstream team can opt into.
...
<CardGroup cols={2}>
  <Card title="Supabase CLI comparison" icon="layer-group" href="/comparisons/supaschema-vs-supabase-cli">
