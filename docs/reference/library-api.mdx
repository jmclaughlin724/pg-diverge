...
| `collectSchemaShapes` / `generateDatabaseTypes` / `generateZodSchemas` | `types` | Collect one schema shape graph, then generate PostgreSQL TypeScript types in the Supabase-compatible `Database` shape plus runtime Zod validators. |
