...
Generated TypeScript and Zod contracts use the same schema model. A database-backed field that remains `unknown` means the source model is missing a relation, function, extension, or expression fact; the fix belongs in the schema source, supported extension model, extractor, or typegen diagnostic path, not in application casts or copied contracts.
