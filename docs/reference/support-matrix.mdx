...
Use this page to check whether supaschema models a PostgreSQL object type or a migration-derived source-intent class.
...
| Side-effect statements | Source-intent only | No direct schema render | `INSERT`, `UPDATE`, `DELETE`, `DO`, and control-plane `SELECT` statements do not belong to the declarative SchemaModel. When present in existing reviewed migrations, they are migration source intent that planning/check/verify must model or preserve; when absent, supaschema must emit an actionable missing-intent diagnostic instead of inventing data work. Schema-source `_bootstrap` directories are treated as provider/bootstrap inventory and skipped by `dir:` and `git:` sources. |
...
| --- | --- | --- |
| Row backfills and data DML | Existing reviewed migrations or an explicit new migration-intent artifact | Preserve, check, and verify the stated operation. Never invent row values, tenant predicates, or backfill criteria. |
