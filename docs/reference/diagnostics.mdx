...
| `SUPA_MIGRATION_INTENT_PARSE_SKIPPED` | warning | A configured migration file could not be fully parsed for source intent; destructive changes in that file may still require explicit hints. |
...
| `SUPA_SYNC_VERIFY_URL_UNRESOLVED` | error | `sync` could not resolve the database URL required by the verify lane. |
