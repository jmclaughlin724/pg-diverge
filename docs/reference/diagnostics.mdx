...
| `SUPA_SYNC_RUNNER_FAILED` | error | The Supabase CLI apply runner launched and exited nonzero. |
| `SUPA_SYNC_RUNNER_UNAVAILABLE` | error | The Supabase CLI apply runner could not be launched because it is not installed or not on `PATH`. |
