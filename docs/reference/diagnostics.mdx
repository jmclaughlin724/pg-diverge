...
| `SUPA_MIGRATION_BASELINE_MISMATCH` | error | The resolved `sources.from` model does not match the generated migration-tree baseline. |
| `SUPA_MIGRATION_BASELINE_UNSUPPORTED` | error | Existing migrations cannot prove a generated-lineage baseline for source-backed generation. |
...
| `SUPA_ROUTINE_DYNAMIC_SQL_DEPENDENCY_HINT_REQUIRED` | error | A relation or type changed while an unhinted routine has unproven dependencies. Add exact `hints.routineDependencies` or split the migration. |
