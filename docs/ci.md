...
- For a standalone drift gate, `pg-diverge diff --from "database:$DATABASE_URL" --to dir:supabase/schemas --fail-on-diff --quiet` exits 3 when the live database and the declarative tree have diverged.
...
- The committed composite action wraps any invocation: `uses: jmclaughlin724/pg-diverge@v0.1.0` with `args: "diff --from database:$DATABASE_URL --to dir:supabase/schemas --fail-on-diff --quiet"` (always pin a tag; the action runs `npx pg-diverge@<version>` with `version` defaulting to `latest`).
