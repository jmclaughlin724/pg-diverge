...
- `lib/` — shared guard transport (`guard-utils.js`, `ast-utils.js`, `sql-ast.js`)
...
- `ci-release/` — `check-ci-governance.mjs`, `check-codex-execpolicy.mjs`, `check-github-process.mjs`, `check-release-version-surfaces.mjs` (Rule 09/19)
- `agent-surface/` — `check-agent-hooks.mjs`, `check-agent-surface-parity.mjs`, `check-agent-policy-standardization.mjs`, `check-claude-agents.mjs`, `check-hook-import-graph.mjs` (Rule 12/22)
