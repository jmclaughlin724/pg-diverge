...
- `components.mjs` — Blume component, card, callout, and image inspection
- `links.mjs` — internal-link classification and image-source validation
- `paths.mjs` — docs path helpers (`hasMarkdownExtension`, `isDigit`)
- `local-runner.mjs` — npm-as-local-runner convention checks across install/setup surfaces

## Owners

- Blume writing and component standards: `.claude/rules/02-*.md`, `.claude/rules/03-*.md`
