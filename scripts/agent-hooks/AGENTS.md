...
- `hook-output.mjs` — runtime-specific event output
...
- Consumer hook templates invoke only the compiled Supaschema generated-artifact and schema-write hooks; the artifact hook's bounded output-target inspection and fail-closed classification are product policy, while general Bash policy remains source-repo-only in `runner.mjs`.
