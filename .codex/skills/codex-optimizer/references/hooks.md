...
2. Keep supaschema project hook matching out of native `matcher` strings. Register one broad command hook per event and route by parsed `tool_name`, `tool_input`, and event payload inside the dispatcher. `UserPromptSubmit` and `Stop` ignore matchers.
...
7. Keep only one hook representation per config layer: prefer `.codex/hooks.json` for supaschema project hooks, not mixed inline `[hooks]` tables plus `hooks.json`.
...
- Codex has no `PostToolBatch` event. When a hook refreshes an entire generated surface after any number of possible writes, prefer `Stop` over per-tool `PostToolUse`; emit valid `Stop` JSON on stdout, and use `decision: "block"` only when Codex should continue the turn to repair a failed sync.
...
- `npm run sync:llm` mirrors `.claude/hooks/**` into `.codex/hooks/**`.
...
- Hook source is Claude-owned and mirrored into `.codex/hooks/**` by `npm run sync:llm`. When Claude and Codex payload or output contracts differ, make the `.claude/hooks/**` source runtime-aware before syncing; do not hand-edit the generated Codex copy.
- For `.claude/**` to `.codex/**` generated-surface synchronization, register the Codex side at `Stop`, not `PostToolUse`. Codex launches multiple matching command hooks for the same event concurrently, so per-tool sync hooks can race a full-surface rewrite against other hook work.
...
- Separate upstream-verified guidance from supaschema adapter policy in hook messages. Do not label repo hardening rules, package paths, or local workflow choices as upstream best practice unless the upstream source directly says so.
...
- For whole-surface sync adapters, batch work by real lifecycle boundary when one command refreshes the whole surface. Use Claude `PostToolBatch` when available and Codex `Stop` when no batch event exists.
- Validation for Codex hook changes should include the shared helper tests, every registered adapter test, `npm run guard:agent`, and JSON parsing for `.codex/hooks.json` when wiring changes.
