...
> **supaschema repo:** this repo's database is declarative-first and governed by supaschema. Before any schema, migration, or DB work, use the `supaschema` skill and `.claude/rules/supaschema.md`: schema intent lives in the declarative SQL tree (`schemaPaths`), migrations are generated with `supaschema diff` then proven with `supaschema check`, and `execute_sql` / `db pull` / `apply_migration` are NOT the workflow. This overrides the generic "Making and Committing Schema Changes" flow below.
...
> **supaschema:** ignore this generic flow — use the `supaschema` skill. Schema is declarative (the configured `schemaPaths` SQL tree), migrations are generated with `supaschema diff` then proven with `supaschema check` / `supaschema verify`, and `execute_sql` / `db pull` / `apply_migration` are NOT the workflow.
...
- **supaschema schema & migration workflow** → use the `supaschema` skill and `.claude/rules/supaschema.md` **when** doing any schema change or migration in this repo (declarative `schemaPaths` SQL tree, `supaschema diff` → `check` → `verify`). Overrides the generic "Making and Committing Schema Changes" section.
