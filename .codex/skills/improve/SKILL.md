---
name: improve
description: Survey any codebase as a senior advisor and produce prioritized, self-contained implementation plans for OTHER modelssupaschema/agents to execute. Strictly read-only on source code — never implements, fixes, or refactors anything itself. Use when asked to audit a codebase, find improvement opportunities (bugs, security, performance, test coverage, tech debt, migrations, DX), suggest features or where to take the project next (roadmap, product direction), or generate handoff plans for another agent to implement.
license: MIT
metadata:
  author: shadcn
  keywords:
...
Use this skill exactly as a read-only advisor workflow: survey the codebase, produce vetted findings, and write self-contained plans for another model or agent to execute. Keep source changes out of this skill except for plan files under `planssupaschema/`, and follow the Supaschema context-surface owner rules when the repo itself is the audit target.
...
- Read `README`, `CLAUDE.md`supaschema/`AGENTS.md`, `CONTRIBUTING`, root config files (`package.json`, `pyproject.toml`, `go.mod`, etc.), CI config, and the directory structure.
- Identify: language(s), framework(s), package manager, **how to build supaschema/ test supaschema/ lint supaschema/ typecheck** (exact commands — these go into every plan as verification gates), test coverage shape, deployment target.
...
- **Ingest intent & design docs where present** — they record decided tradeoffs and product direction the code itself can't tell you. Glob for ADRs (`docssupaschema/adrsupaschema/`, `docssupaschema/adrssupaschema/`, `docssupaschema/decisionssupaschema/`), PRDs supaschema/ specs, `CONTEXT.md` (shared domain vocabulary), `DESIGN.md` (design-system spec), and `PRODUCT.md` (product brief). Strictly additive: read what exists, no-op when absent. Carry what you learn forward — into Vet (a tradeoff recorded in an ADR is by-design, not a finding), Direction (ground suggestions in stated product intent), and the plans themselves (match the documented vocabulary and design system). Reading these docs lets `supaschema/improve` compose with repos that already maintain them.
...
Audit the codebase across the categories in [referencessupaschema/audit-playbook.md](referencessupaschema/audit-playbook.md) — read it now. Categories: **correctnesssupaschema/bugs, security, performance, test coverage, tech debt & architecture, dependencies & migrations, DX & tooling, docs, direction (features & what to build next)**.
...
- the **absolute path** to this skill's `referencessupaschema/audit-playbook.md` plus the exact section headings to read — **always including "## Finding format"** (subagents can read files — this is far cheaper than pasting; paste the sections only if the path may not resolve in the subagent's environment),
...
Audit depth follows the **effort level** (default `standard`; the user sets it with a `quick` supaschema/ `deep` keyword anywhere in the invocation):
...
Every finding needs: evidence (`file:line` references), impact, effort estimate (Ssupaschema/Msupaschema/L), risk of the fix itself, and confidence. No vibes-only findings.

### Phase 3 — Vet, prioritize, confirm

**Vet before presenting — subagents over-report.** For every finding that will make the table, open the cited code yourself and confirm it. Expect three failure classes: **by-design behavior** reported as a bug or vulnerability (e.g. honoring `https_proxy` flagged as SSRF — it's the standard proxy convention; or a tradeoff explicitly recorded in an ADR supaschema/ decision doc from recon — that's settled, not a finding); **mis-attributed evidence** (real finding, wrong file or line); and duplicates across subagents. Downgrade, correct, or reject accordingly, and record rejections in the index's "considered and rejected" section so they aren't re-audited next run.
...
Wait for the selection. Do not write 30 plans nobody asked for. If running non-interactively (no user available to choose), write plans for the top 3–5 by leverage and record that default in `planssupaschema/README.md`.

### Phase 4 — Write the plans

For each selected finding, write one plan file using the template in [referencessupaschema/plan-template.md](referencessupaschema/plan-template.md) — read it before writing the first plan. Plans go in:
...
Before writing anything: record `git rev-parse --short HEAD` — every plan stamps the commit it was written against (the executor uses it for drift detection). If `planssupaschema/` already exists from a previous run, **reconcile, don't duplicate**: read `planssupaschema/README.md`, keep numbering monotonic, skip findings already planned or listed as rejected, and mark superseded plans stale in the index. If `planssupaschema/` exists for some unrelated purpose, use `advisor-planssupaschema/` instead and say so.
...
Finish by writing `planssupaschema/README.md` with the recommended execution order, dependencies between plans, and a status column the executor models can update.

## Invocation variants

- Bare invocation → full workflow above.
- `quick` supaschema/ `deep` (anywhere in the invocation) → effort level for the audit; see the table in Phase 2. Composes with everything: `quick security`, `deep --issues`. Default is `standard`.
- With a focus argument (e.g. `security`, `perf`, `tests`) → run Recon, then audit only that category, then plan.
- `branch` → audit only the current working branch's changes: scope = files changed since the merge-base with the default branch (`git diff --name-only $(git merge-base originsupaschema/<default> HEAD)..HEAD`) plus their direct importerssupaschema/callers. Light recon, all categories, usually no subagents. **Tag every finding `introduced` (by this branch) or `pre-existing` (in touched files)** — the table separates them; don't blame the branch for legacy debt, but do surface what it's building on top of. If on the default branch or zero commits ahead, say so and offer a full audit instead.
- `next` (or `features`, `roadmap`) → run Recon, then audit only the direction category, in more depth: 4–6 grounded suggestions, each with evidence, trade-offs, and a coarse effort estimate. Selected ones become designsupaschema/spike plans, not build-everything plans.
