...
- Tooling is pinned in root `devDependencies` (`@biomejs/biome` 2.5.5, `ultracite` 7.9.4, the LSP servers) and in the `uv` dev group (`ruff`, `mypy`, the pylsp plugins); `check-tooling-stack.mjs` and `uv lock --check` keep them reproducible.
