...
- **Biome via Ultracite** owns JavaScript, TypeScript, JSX, TSX, JSON, JSONC, CSS, HTML, and GraphQL formatting/linting through `biome.jsonc` (extends `ultracite/biome/{core,type-aware,vitest}`) — see Rule 08 for the JS/TS lint policy. Use `npm run format` for write/fix remediation; its Biome step runs `ultracite fix .` to apply Biome format, safe lint fixes, and `organizeImports`. The read-only gate is `npm run lint` (`ultracite check .`) and CI runs `npm run lint:ci` (`biome ci .`).
...
- Run `npm run format` to move formatting, lint-fix, import-sort, and key-sort work forward; do not use `npm run lint` as a fixer and do not append `fix` to `npm run lint`.
