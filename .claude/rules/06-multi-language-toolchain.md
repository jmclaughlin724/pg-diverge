...
- lefthook `pre-commit` formats staged files across every language owner in parallel with `stage_fixed: true` (fixes are re-staged): Biome (`biome check --staged --no-errors-on-unmatched`) on JS/TS/JSON/CSS/HTML/GraphQL, Prettier on MD/MDX/YAML, taplo on TOML, and the `sh-syntax` port on shell; empty-glob lanes are skipped, so a commit touching one language runs only that lane. `pre-push` runs `npm run typecheck` and `npm run guard` (which includes the cclsp coverage and tooling-stack guards).
...
- Tooling is pinned in root `devDependencies` (`@biomejs/biome` 2.5.1, `ultracite` 7.8.3, the LSP servers) and in the `uv` dev group (`ruff`, `mypy`, the pylsp plugins); `check-tooling-stack.mjs` and `uv lock --check` keep them reproducible.
...
Use `npm run format` for write/fix remediation. Public clean-checkout verification accepts `npm run guard:lsp-coverage` returning `LSP_COVERAGE_SKIPPED_LOCAL_ONLY`.
