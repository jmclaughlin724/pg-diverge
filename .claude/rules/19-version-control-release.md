...
3. Add or update the top `CHANGELOG.md` entry as `## <version> (YYYY-MM-DD)` with concise release notes.
...
6. Run `npm run guard:release-version` before broader release or package checks.
...
- `npm run guard:release-version` (`scripts/guards/check-release-version-surfaces.mjs`) asserts that:
  ...
  - the first `CHANGELOG.md` version heading matches the package version;
...
All version-coupled surfaces agree with the package version, the changelog top entry names that version, local release-version guards pass, and requested git/PR actions are completed without touching unrelated work.
