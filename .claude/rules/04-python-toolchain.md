...
- `.github/workflows/python.yml` is the Python CI lane (triggered on `services/**`, `pyproject.toml`, `uv.lock`, and the workflow file). It runs, in order: `uv sync --locked` (lockfile freshness), then the `py:*` command bodies — `uv run --package supaschema-agent-mcp ruff check services/agent-mcp`, `uv run --package supaschema-agent-mcp ruff format --check services/agent-mcp`, `uv run --package supaschema-agent-mcp mypy services/agent-mcp/supaschema_agent_mcp`, `uv run --package supaschema-agent-mcp pytest services/agent-mcp/tests` — then `uvx pip-audit`. The `--package supaschema-agent-mcp` selector is mandatory: the workspace root has no runtime deps (`dependencies = []`, `package = false`) and does not depend on the member, so a bare `uv run mypy`/`pytest` resolves in the root env that lacks `fastmcp`/`mcp`/`pydantic` and fails with `import-not-found`. The CI lane mirrors the canonical `py:*` npm scripts above; it must not re-spell their invocations with different flags.
