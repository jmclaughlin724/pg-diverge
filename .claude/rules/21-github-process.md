---
description: GitHub repository settings, pull-request preflight, merge method, branch cleanup, and post-merge verification.
paths:
  - ".github/**"
  - "scripts/github/**"
...
This rule owns the GitHub lifecycle after local work exists: repository merge settings, `main` branch protection, PR preflight, merge preflight, canonical merge method, branch cleanup, and post-merge verification.

Rule 09 owns GitHub Actions workflow posture. Rule 14 owns local edit/worktree/git safety and the main-agent-only staging, commit, and push boundary. Rule 19 owns release-version transactions. This rule owns remote GitHub process state and the commands that prove a PR can be reviewed, merged, and verified without hidden conflicts or omitted checks.
...
- Required status checks MUST include `quality (22)`, `quality (24)`, `check (15)`, `check (16)`, `check (17)`, `check-os (macos-latest)`, `check-os (windows-latest)`, `dependency-review`, `analyze (javascript-typescript)`, and `analyze (python)`.
- Required status checks MUST be bound to the GitHub Actions app source.
...
- Review settings MUST match `.github/repo-policy.json`; if required review count changes, update the policy, CODEOWNERS expectations, and PR template in the same change.
...
- The ruleset MUST block deletion and non-fast-forward updates, require linear history, require pull requests, require resolved review threads, and require the same GitHub Actions status checks as branch protection.
...
The PR MUST target `main`, MUST be open, MUST not be draft, MUST have `mergeStateStatus: CLEAN`, MUST not have requested changes, MUST have every required status check in `.github/repo-policy.json` with `SUCCESS`, `NEUTRAL`, or `SKIPPED`, and MUST have no unresolved review threads when conversation resolution is required.
...
Fix the branch, PR, policy, GitHub setting, or required check that failed. Do not bypass branch protection, use admin merge to skip checks, change the PR base to avoid conflicts, disable a failing required check, or loosen `.github/repo-policy.json` to make a merge pass.

## Done means

The repo policy, live GitHub settings, PR template, local guard, preflight scripts, required checks, merge method, branch cleanup, and post-merge verification all agree.
