...
This rule owns structural analysis policy for repo scripts and hooks: use an AST/parser for code or SQL structure; reserve regex for text patterns with an explicit `regex-ok` reason.
...
Run `npm run guard` or `npm run guard:no-regex-in-scripts` after changing scripts, hooks, guards, or parser helpers. Detector changes need focused true-positive and false-positive tests.

## Failure behavior

Convert structural regexes to TypeScript AST, libpg-query, JSON/TOML/YAML parsers, or literal string operations as appropriate. If regex remains, add a narrow `regex-ok` reason that explains why the target is text, not structure.

## Done means

The classifier uses the canonical parser, tests cover the target structure, and any remaining regex is text-only and justified.
