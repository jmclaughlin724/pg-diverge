...
| Git | `git checkout`, non-transactional `git switch`, direct `git branch`, ad hoc CLI `git worktree`, `git reset`, `git restore --source`, `git stash`, local `git merge --squash`, destructive branch operations, and force-push without explicit approval. | Rule 14; Bash blocker; branch preflight checks. |
...
| Git | Committing PR or feature-branch work on local `main`, then pushing it to a branch ref or using a realignment recovery. Establish explicit PR intent and enter a topic checkout before the first commit through host-managed isolation or Rule 14's transactional switch path. | Rules 14 and 21; Bash blocker; `git status --short --branch`. |
