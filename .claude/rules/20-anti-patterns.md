...
| Git | Committing PR or feature-branch work onto local `main` and pushing it to a branch ref, leaving local `main` diverged from `origin/main`. Create the branch first with the worktree tool (`EnterWorktree`, branched from `origin/main`). | Rules 14 and 21; `git status --short --branch`. |
