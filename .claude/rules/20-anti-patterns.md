...
| Verification | Claiming GitHub, CI, PR, branch, or checks are green without successful `github-checks` evidence, or while failed `statusCheckRollup` evidence remains unresolved. | Rules 12 and 21; recorded command evidence; review. |
| Agent response shape | Mechanism-only correctness answers: explaining runtime or upstream-valid behavior without the `$elegant` architecture/end-state disposition and verification disposition. | Rules 01 and 12; review. |
| Change discipline | Minimal-patch bias and backwards-compatibility preservation: keeping duplicate owners, re-export shims, alias modules, wrappers, export-only compatibility files, DTOs, facades, copied enum tuples, cast-based contract patches, local view-models, local compatibility layers, broader helper surfaces, allowlist exceptions, transitional branches, redundant docs, convenience entry points, placeholders, TODOs, or multiple entry points instead of consolidating into the canonical owner and rewriting or deleting consumers. | Rule 01 `$elegant`; `scripts/guards/code-shape/check-canonical-surfaces.mjs`; `npm run guard`. |
...
| Change discipline | Typed UI prop containers that rename, project, mirror, or locally re-own DB-backed generated contracts instead of using the generated contract directly. | Rule 01 `$elegant`; generated type checks. |
...
| Worktree | Reverting, stashing, resetting, cleaning, or overwriting unrelated tracked or untracked work. | Rule 21; `git status --short`; diff review. |
...
| Git | `git checkout`, non-transactional `git switch`, direct `git branch`, ad hoc CLI `git worktree`, `git reset`, `git restore --source`, `git stash`, local `git merge --squash`, destructive branch operations, and force-push without explicit approval. | Rule 21; Bash blocker; branch preflight checks. |
| Git | `git commit --no-verify`, `git push` as a diagnostic probe, and any direct push to `main`. | Rules 14, 19, and 21; Bash blocker; protected main ruleset. |
| Release | Merging, pushing to `main`, or publishing a versioned release without running `$update` after the final release-owned changes and resolving its in-scope findings. | Rule 19; `$update`; `docs/release.mdx`; release PR checklist; review of owner-validation evidence. |
...
| Git | Continuing work on a squash-merged topic branch, treating its expected ahead count as unmerged content, or leaving its local or remote-tracking ref after merge instead of completing branch cleanup. | Rule 21; post-merge closeout; `git fetch --prune origin`; PR-state and ref evidence. |
| Git | Committing PR or feature-branch work on local `main`, then pushing it to a branch ref or using a realignment recovery. Establish explicit PR intent and enter a topic checkout before the first commit through host-managed isolation or Rule 21's transactional switch path. | Rule 21; Bash blocker; `git status --short --branch`. |
| PR review | Resolving a PR review thread or dismissing a failing check before its correction is committed, resolving a valid unaddressed finding, or acting on a review suggestion without first verifying it against upstream/canonical sources. | Rules 21 and 05; `required_conversation_resolution`; `npm run guard:github-process`. |
...
| Database | Hand-editing generated migrations, overwriting generated migration files outside `supaschema diff --replace`, bypassing lineage, using `--no-check-chain` without approval, or applying through `sync` without configured automatic sync approval or an explicit user-approved override target. | `supaschema.md`; migration hooks; supaschema guards. |
...
| CI/release | Unpinning actions, adding stored npm publish tokens, weakening OIDC/provenance, blocking release completion on npm registry propagation, path-skipping required gates, moving DB proofs into release, bypassing hooks, or publishing release notes from generated notes instead of the changelog. | Rules 09 and 19; CI and release guards. |
