...
- Consumer setup is one package install with the consuming project's package manager. The agent install protocol lives at `.agents/prompts/supaschema-install.md`; it owns package-manager selection, workspace targeting, local runner commands, lifecycle-script recovery, and wrong-manager stop conditions. The installed config, schema/migration directories, supaschema consumer rule/skill/hook bundle, hook wiring, and tagged `AGENTS.md` / `CLAUDE.md` addendum are part of the package surface. Maintainer-only Claude/Codex optimization and context-enforcement tooling remains repo-local under Rule 13. `postinstall` and `supaschema init` perform consumer setup through one shared scaffolder (`bin/scaffold.mjs`), so `supaschema init` is at full parity with `postinstall` — config, directories, agent bundle, hook wiring, and pending path-confirmation state when needed — not a config-only recovery step. It is the explicit fallback when dependency lifecycle scripts are blocked or skipped by npm, pnpm, Yarn, Bun, or local project policy. It is idempotent: existing JSON config files are left untouched unless explicit repair is requested, JavaScript config files are not loaded or converted, and the managed guidance block is upserted in place.
...
- `supaschema.config.json` owns `adapter`, `workflow`, `schemaPaths`, `sources`, `migrationsDir`, `typesFile`, `zodFile`, `managedSchemas`, `transactionMode`, and named `environments`. `sources.from` and `sources.to` are the machine-readable defaults for zero-source-flag `diff`, `plan`, and `verify`; install writes `sources.from: "auto"` and `sources.to: "dir:<schemaPaths[0]>"`. `sources.from: "auto"` resolves a database URL, then valid `git:HEAD`, then `empty:` for a first migration in a fresh repository. `adapter: "auto"` is provider-neutral and is not a Supabase switch. Workflow automation is explicit: default `workflow.schema_diff: "on_schema_write"` runs hook diff, `workflow.migration_check: "after_schema_diff"` runs hook check, `workflow.type_generation: "create_or_refresh"` and `workflow.zod_generation: "create_or_refresh"` make `diff` create or update generated TypeScript and Zod outputs, `workflow.type_usage: "zod_validated"` tells agents to use generated Zod validators as the runtime-boundary contract, `workflow.migration_verify` controls verify guidance, and `workflow.migration_sync` controls explicit sync apply handoff. Provider-specific behavior is expressed through paths, managed schemas, transaction mode, excluded grant roles, and explicit verify flags. Generic PostgreSQL installs use `managedSchemas: []`; Supabase installs seed the Supabase platform schema list.
...
- When the bundled PostToolUse hook is wired and workflow defaults are kept, a write to a schema-tree `.sql` file auto-runs `supaschema diff` then `supaschema check`; `diff` also creates or refreshes `typesFile` and `zodFile` under the default output policies. The hook returns the generated migration name, generated-output policy, or blocking `SUPA_*` diagnostic as agent context. If `check` fails, the hook emits loop-continuation feedback; inspect the diagnostic, identify the canonical root source in the declarative tree or generated migration chain, search for similar or correlated migration failures, fix the canonical owner, regenerate when the tree changes, and rerun `supaschema check`. Treat hook context as the authoritative diff result, resolve any reported `SUPA_*` code before continuing, and do not re-run the diff by hand to "confirm" a clean result. The hook generates and proves; it never applies to a database, and it skips multi-root or path-confirmation cases that require human review.
...
- `supaschema sync` is an explicit operational handoff. With no apply flag it is a dry run; with `--local` or `--remote` it runs migration status and replay-safety gates, then delegates the actual apply/deploy to the Supabase CLI only when `workflow.migration_sync` is `explicit_request_only`. Agents must not use apply flags without an explicit human request.
...
Resolve `SUPA_*` diagnostics through `supaschema explain`, edit the declarative tree rather than generated migrations, and ask for explicit approval before any `sync --local|--remote` apply flag or `--no-check-chain` path.
