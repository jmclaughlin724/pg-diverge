...
- `supaschema.config.json` owns `adapter`, `workflow`, `sync.targets`, `schemaPaths`, `sources`, `migrationsDir`, `typesFile`, `zodFile`, `managedSchemas`, `transactionMode`, and named `environments`. `sources.from` and `sources.to` are the machine-readable defaults for zero-source-flag `diff`, `plan`, and `verify`; install writes `sources.from: "auto"` and `sources.to: "dir:<schemaPaths[0]>"`. `sources.from: "auto"` resolves a database URL, then valid `git:HEAD`, then `empty:` for a first migration in a fresh repository. `adapter: "auto"` is provider-neutral and is not a Supabase switch. Workflow automation is explicit: default `workflow.schema_diff: "on_schema_write"` runs hook diff, `workflow.migration_check: "after_schema_diff"` runs hook check, `workflow.type_generation: "create_or_refresh"` and `workflow.zod_generation: "create_or_refresh"` make `diff` create or update generated TypeScript and Zod outputs, `workflow.type_usage: "zod_validated"` tells agents to use generated Zod validators as the runtime-boundary contract, `workflow.type_safety` and `workflow.rls_safety` default to deploy-blocking sync safety gates, `workflow.migration_verify` controls verify guidance, and default `workflow.migration_sync: "auto"` allows bare `supaschema sync` to select targets by `sync.targets.<name>.mode`. Target config, not workflow, decides which local, staging, preview, or remote targets are selected. Provider-specific behavior is expressed through paths, managed schemas, transaction mode, excluded grant roles, sync targets, and explicit verify flags. Generic PostgreSQL installs use `managedSchemas: []`; Supabase installs seed the Supabase platform schema list.
