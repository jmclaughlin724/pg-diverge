...
- `supaschema.config.json` owns `adapter`, `schemaPaths`, `migrationsDir`, `typesFile`, `zodFile`, `managedSchemas`, `transactionMode`, and named `environments`. `adapter: "auto"` is the default automated supaschema workflow for generating migrations, checking/verifying them, and refreshing existing type outputs; it is not a Supabase switch. Provider-specific behavior is expressed through paths, managed schemas, transaction mode, excluded grant roles, and explicit verify flags.
