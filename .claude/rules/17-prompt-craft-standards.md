...
- Use programmatic tool calling only for a bounded deterministic reduction stage such as filtering, joining, ranking, deduplication, aggregation, batching, or repeated validation. State the eligible tools, output schema, retry limit, stop condition, and handoff.
...
- Lead final responses with the outcome. Preserve required evidence, decisions, material caveats or blockers, and the next useful action; trim repetition, generic reassurance, introductions, and optional background first.
