...
- Tarball smoke and registry smoke are separate package proofs. `npm run package:smoke` proves the local tarball installs and initializes through supported package managers before publish. The release workflow must also run a post-publish registry smoke against the just-published `supaschema@<version>` so npm, pnpm, and Bun can download and execute the package from the npm registry.
- Keep package-manager smoke on current supported client behavior. pnpm coverage must include pnpm 11 behavior and must use an explicit `minimumReleaseAge` override for immediate-release smoke paths instead of accidentally accepting pnpm's default one-day fresh-version cooldown as package unavailability. Consumer installation docs must keep `pnpm add supaschema` as the default so ordinary installs retain pnpm's release-age protection; document `--config.minimumReleaseAge=0` only for immediate-release troubleshooting.
