...
STOP if root `.npmignore` is introduced, a maintainer-only support surface enters `package.json` `files`, a consumer agent surface is removed from the tarball, a `.tsbuildinfo` or other build cache appears in the dry-run tarball, or `postinstall` writes repo-only tooling into a consuming project without a documented contract change.
