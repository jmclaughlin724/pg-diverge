...
- `supaschema` exposes exactly three tools: `server_status`, `code_atlas_query`, and `repo_context_query`. Rule 10 owns the Code Atlas query contract. `repo_context_query` reads and searches non-secret repo files on a **deny-list** model (AGENTS instructions, rules, skills, source, schemas, tooling config) and returns nearest agent instructions. Secrets/credentials (`.env*`, `*.key`/`*.pem`/`*.p12`/`*.pfx`, `secrets/`), `.git`, caches/build output, and archived `plans/` stay blocked. It must not expose raw SQL, arbitrary shell execution, DB/API mutation, credential reads, external LLM calls, or proxy calls to other MCP servers.
