...
- When present, the `supaschema` repo-context server is a supplementary context shortcut, not a repo graph or final proof. Repository instructions, manifests, configured AST/LSP tooling, direct source reads, focused tests, and guards remain the reproducible evidence.
...
FastMCP surfaces, config, capability index, tests, and guards agree; `supaschema` stays read-only, non-secret, and supplementary to direct repository evidence.
