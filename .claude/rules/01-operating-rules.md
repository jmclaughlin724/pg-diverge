...
- **Burn down duplicates in scope.** MUST follow the `AGENTS.md` Repo-Wide Change Discipline: identify one owner and one entry point, collapse avoidable duplicate or redundant surfaces in the accepted scope, and use upstream evidence plus the elegant end state when the shape is not obvious. VERIFY with Code Atlas owner and entrypoint queries plus direct source reads. FIX BY merging, moving, deleting, or documenting the distinct boundary that justifies a separate surface.
...
STOP if any STOP condition in `.claude/rules/*` is skipped, a guard or test is weakened instead of its cause being fixed, a standard ships without an executable enforcement path, the package manager is switched away from npm, a technical decision ships on a guess when an authoritative upstream source was available, or avoidable duplicate or redundant surfaces or multiple entry points remain in the accepted scope.
