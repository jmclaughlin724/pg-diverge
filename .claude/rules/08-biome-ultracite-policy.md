...
- Markdown and YAML have no Biome/Prettier auto-format gate in this repo (no Prettier is installed). `docs/` Markdown is gated by `npm run docs:lint` + `mint validate` (Rules 02/03); other Markdown/YAML is reviewed, not auto-formatted. See Rule 06 for the per-language owner map.
...
- When adding a governed root file or expanding the lint surface, run `npm run lint:fix` before broad acceptance so new JSON/MJS/config surfaces are formatted before `npm run lint` reaches them.
