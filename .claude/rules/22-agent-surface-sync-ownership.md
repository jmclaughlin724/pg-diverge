...
- Source-repo Codex hook commands MUST resolve repo-local entrypoints from the Git root: POSIX commands use `$(git rev-parse --show-toplevel)`, and every handler MUST provide an equivalent `commandWindows` override. Consumer general-guard handlers MUST preserve the same cross-platform root resolution. Commands MUST NOT rely on undocumented `CODEX_PROJECT_DIR` injection or the session working directory.
