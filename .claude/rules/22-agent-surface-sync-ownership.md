...
- Source-repo Codex hook commands MUST resolve repo-local entrypoints from the Git root: POSIX commands use `$(git rev-parse --show-toplevel)`, and every handler MUST provide an equivalent `commandWindows` override whose loop body suppresses command echo so hook stdout remains protocol-only JSON. Consumer general-guard handlers MUST preserve the same cross-platform root resolution and echo suppression. Commands MUST NOT rely on undocumented `CODEX_PROJECT_DIR` injection or the session working directory. Once launched, a mirrored hook MUST derive its canonical project root from its own entrypoint path and use the payload `cwd` only to resolve relative tool targets.
- `scripts/agent-hooks/runner.mjs` MUST own source-repo Claude and Codex `PreToolUse` dispatch order: repository boundary first, skill/context gate, response-evidence gate, then Bash safety through `.claude/hooks/guards/bash-policy-checks.mjs`. The repository boundary MUST cover every supported local tool, deny paths outside the canonical project root, deny linked worktrees and all worktree commands, and deny branch creation or switching without explicit current-prompt authorization. Rule 12 owns the Stop-time evidence gate that blocks green claims while failed `github-checks` evidence remains unresolved.
...
- Consumer Claude hook templates MUST use exec-form `command: "node"` plus `args` for Node hook entrypoints containing `${CLAUDE_PROJECT_DIR}`. Package-manager and package-bin shims MUST use shell-form `command` strings without `args` so Windows `.cmd` or `.bat` shims remain executable.
...
- Every supported local source-repo Claude and Codex tool matches exactly one context `PreToolUse` hook command, and the shared runner supplies repository confinement, worktree denial, explicit branch authorization, and Bash safety.
