...
- The Python lane installs with `astral-sh/setup-uv` and `enable-cache: true`, and `uv sync --locked` is the lock-freshness gate (fails on `uv.lock` drift). CI/docs Node lanes use `actions/setup-node` with npm caching where appropriate; the privileged release job disables package-manager caching to avoid unnecessary package-manager side effects on the OIDC publish path. There is no external/remote build cache in this repo.
