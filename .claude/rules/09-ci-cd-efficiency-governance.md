...
- `docs.yml`: path-filtered (`docs/**`, `package.json`, `package-lock.json`, the workflow itself) docs validation via `npm run docs:check`.
...
- `actions/checkout` uses `persist-credentials: false` on every tracked workflow's checkout (`ci.yml`, `consumer-canary.yml`, `dependency-review.yml`, `docs.yml`, `python.yml`, `release.yml`, `scorecard.yml`, `snapshot.yml`); release re-supplies an explicit `GH_TOKEN`/`registry-url` only where a step needs it.
- Every job declares a `timeout-minutes` ceiling so no lane can hang at the 360-minute default, least of all the privileged release job (`ci.yml` 20/30/20/5, `consumer-canary.yml` 30, `dependency-review.yml` 10, `docs.yml` 15, `python.yml` 15, release preflight/publish 5/30, `scorecard.yml` 20, `snapshot.yml` 30).
...
- Commit signoff enforcement MUST NOT be wired into CI quality jobs; Rule 21 owns the signoff policy and its approval path.
...
- The eight workflow files themselves under `.github/workflows/` (`ci.yml`, `consumer-canary.yml`, `dependency-review.yml`, `docs.yml`, `python.yml`, `release.yml`, `scorecard.yml`, `snapshot.yml`), plus `.github/dependabot.yml`.
