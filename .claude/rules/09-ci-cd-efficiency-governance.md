...
- `npm run guard:ci` (`scripts/guards/ci-release/check-ci-governance.mjs`, part of `npm run guard`) — parses every tracked workflow with the `yaml` dependency and asserts this rule's STOP invariants deterministically: top-level least privilege, SHA pins, third-party action allowlist coverage against `.github/repo-policy.json`, credential handling, PR concurrency, the stable `CI required` aggregation job, read-only vulnerability-only Dependency Review, release queueing, read-only release preflight, conditional write/OIDC entry, block-mode egress endpoints, trusted publishing, exact tarball smoke/publish, provenance attestation, changelog-owned GitHub Release creation, supported runtime matrices, single-owner examples, and tracked Python workflow presence.
