...
- The privileged `release.yml` `publish` job runs `step-security/harden-runner` with `egress-policy: block` and the endpoint baseline verified from the 0.4.0 trusted-publishing run. Keep GitHub Actions OIDC/results endpoints, GitHub API/git, npm and GitHub Packages registries, and Sigstore Fulcio/Rekor/TUF endpoints explicit. Do not weaken the publish job back to audit-only egress.
