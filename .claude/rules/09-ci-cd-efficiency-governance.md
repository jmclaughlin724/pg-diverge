...
- Every job declares a `timeout-minutes` ceiling so no lane can hang at the 360-minute default — least of all the privileged release job (`ci.yml` 20/30/20/5, `dependency-review.yml` 10, release preflight/publish 5/30, `codeql.yml`/`scorecard.yml` 20, `docs.yml` 15).
...
- Concurrency is declared in each PR-facing workflow (`ci.yml`, `codeql.yml`, `dependency-review.yml`, `docs.yml`) as `group: "${{ github.workflow }}-${{ github.ref }}"` with `cancel-in-progress: true`, so superseded PR runs are cancelled. `release.yml` queues every release transaction and never cancels an in-flight publish.
...
- Repository Actions policy allows GitHub-owned actions plus the exact reviewed third-party action repositories used by the six workflows; full-SHA pinning remains mandatory. Do not restore `allowed_actions: all`.
...
- `npm run guard:ci` (`scripts/guards/ci-release/check-ci-governance.mjs`, part of `npm run guard`) — parses every tracked workflow with the `yaml` dependency and asserts this rule's STOP invariants deterministically: top-level least privilege, SHA pins, credential handling, PR concurrency, the stable `CI required` aggregation job, CodeQL matrices, read-only allow-listed Dependency Review, release queueing, read-only release preflight, conditional write/OIDC entry, block-mode egress endpoints, trusted publishing, exact tarball smoke/publish, provenance attestation, changelog-owned GitHub Release creation, supported runtime matrices, single-owner examples, and private Python workflow exclusion.
...
STOP if a tracked workflow grants top-level write permissions, unpins an action, restores unrestricted action admission, drops `persist-credentials: false`, permits direct `main` updates without the stable CI/CodeQL/dependency checks, lets a no-op release enter the write/OIDC job, weakens Harden-Runner from block mode, replaces OIDC with a stored npm token, drops release provenance/version gates, publishes from an unprotected source, removes a supported runtime matrix, drops a scanner, removes stale-run cancellation from a PR workflow, or shares one migration output directory across independent renders.
