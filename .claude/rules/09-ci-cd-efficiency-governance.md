...
- `actions/checkout` uses `persist-credentials: false` on jobs that do not need the checkout token to persist (all of `ci.yml`, `python.yml`, `dependency-review.yml`, `scorecard.yml`, `release.yml`); release re-supplies an explicit `GH_TOKEN`/`registry-url` only where a step needs it.
...
STOP if a workflow grants top-level write permissions, unpins an action from its full-length commit SHA, drops `persist-credentials: false` on a job that does not need persisted credentials, removes harden-runner from the OIDC publish job, replaces OIDC trusted publishing with a stored npm token, removes a supported Node/Postgres/OS matrix lane, moves database-backed proofs out of the `postgres`-service `check` job, removes concurrency cancellation from a PR-facing workflow, or disables a supply-chain scanner (`npm audit signatures`, dependency-review, `pip-audit`, CodeQL, or Scorecard).
