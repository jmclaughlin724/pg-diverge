...
- Rules should remain durable policy with scoped `paths` where useful. Executable shell allow/prompt/forbid behavior belongs in Codex-native rule files; otherwise Codex rule files should point at the canonical rule owner instead of duplicating long-form policy.
