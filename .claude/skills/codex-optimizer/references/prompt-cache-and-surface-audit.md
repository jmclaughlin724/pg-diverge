...
- Long-running agent prompts should require persistence through implementation and verification, use preambles for notable tool work, and track progress with a TODO/rubric only when that prevents missed work.
...
- Use `prompt_cache_key` consistently for API traffic that shares common prefixes. Choose a granularity that keeps each prefix/key combination under roughly 15 requests per minute, and preserve `usage.prompt_tokens_details.cached_tokens` in logging or metering when available.
...
- 268 hook source/test files under `.claude/hooks/**` and `.codex/hooks/**`.
- 80 canonical `.claude/skills/**/SKILL.md` files.
- 63 canonical `.claude/rules/**` files.
- 60 non-worktree `AGENTS.md` files.
- 16 `.claude/agents/**` definitions.

The review intentionally excluded `.claude/worktrees/**` worker checkouts and generated mirrors such as `.agents/**`, `.codex/rules/**`, `.codex/agents/**`, and `.gemini/**` except when checking sync ownership.
...
- Large workspace `AGENTS.md` files are allowed when they are true owner briefs. Keep current durable owner state there, move reusable workflows into `.claude/skills/**`, and move durable cross-owner policy into `.claude/rules/**`.
...
- Rules should remain durable policy with scoped `paths` where useful. Executable shell allow/prompt/forbid behavior belongs in Codex rules generated from the canonical rule owner, not in long-form instruction prose.
- Reasoning effort, verbosity, prompt document byte limits, tool output limits, and instruction-file overrides are runtime config levers. Change them in `.codex/config.toml` only when the request is explicitly about Codex runtime behavior or latency/cost/reliability tradeoffs.
...
- The temporary pattern of workspace `AGENTS.md` files pointing to sibling `references/agents-detail.md` files was retired. Current owner state now belongs directly in the local `AGENTS.md`; workflows and policy move to skills or rules.
- Oversized `SKILL.md` bodies were converted to stable skill contracts plus `references/skill-playbook.md` detail, preserving frontmatter trigger metadata, for `ai-elements`, `ai-sdk`, `cloudflare`, `cloudflare-agents-sdk`, `cloudflare-building-ai-agent`, `cloudflare-building-mcp-server`, `cloudflare-durable-objects`, `cloudflare-web-perf`, `cloudflare-workers-best-practices`, `cloudflare-wrangler`, `debugger`, `safe-action`, `task-creator`, `turborepo`, `playwright-cli`, `anilize-apps-portal`, `anilize-apps-sites`, `adversarial-verification`, `supabase`, `worker-prompt-craft`, `remotion`, `team`, `observability`, `calculator-add`, `plaid`, `update`, `together-chat-completions`, `prompt-creator`, and `arive`.
- The temporary custom-agent playbook-directory pattern was retired. Custom agents now keep bounded mission/workflow/output contracts directly in `.claude/agents/**`; repeatable multi-step procedures move to skills when they outgrow the agent prompt.
- Short Playwright subagents were normalized in place with `#` title, `## Mission`, `## Workflow`, and output/quality sections.
...
- Mechanical audits and rewrites used Markdown AST and TypeScript/structured parsing. Do not reintroduce regex-based source classification for this surface-audit workflow.
...
2. Decide whether the finding is a prompt-cache shape issue, a trigger-quality issue, a parser/enforcement issue, a runtime config issue, or a sync ownership issue.
...
5. If the finding affects generated mirrors, patch the canonical `.claude/**` owner or root brief first, then run the matching sync command.
