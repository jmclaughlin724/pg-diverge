...
1. **Scope before choosing commands.** Use Code Atlas first for repo-wide owner/impact/consumer/generated-surface evidence: `npm run code-atlas:query pre-edit <file>`, `npm run code-atlas:query impact <file>`, `npm run code-atlas:query entrypoints <filter>`, `npm run code-atlas:query health <filter>`; when the `codeatlas` MCP is exposed, use the Code Atlas skill's tool map before broad source reads.
