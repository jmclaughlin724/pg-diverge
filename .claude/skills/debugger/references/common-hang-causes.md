...
- **FastMCP stdio server idle**: `repo_context` is read-only stdio — "no output" is normal until a tool is called. Probe it with `npm run fastmcp:status` / `fastmcp:inspect` rather than assuming it is dead.
