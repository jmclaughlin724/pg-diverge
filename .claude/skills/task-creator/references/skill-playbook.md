...
2. Use the relevant MCP server whenever it can validate a planning claim better than memory.
   - If the `codeatlas` MCP server is exposed, use the Code Atlas skill's MCP tool map for live entrypoint packs, pre-edit briefs, impact analysis, health reports, and snapshot queries; still reproduce plan-owned inventory with `pnpm code-atlas:query` because it is guardable in this repo.
3. For Next.js work:
