...
Use this skill exactly as a read-only advisor workflow: survey the codebase, produce vetted findings, and write self-contained plans for another model or agent to execute. Keep source changes out of this skill except for plan files under `plans/`, and follow the Anilize context-surface owner rules when the repo itself is the audit target.
