...
**Explicit `/skill` tokens:** The matcher recognizes `/skill-name` tokens both at the start of the prompt (leading tokens) and mid-sentence (e.g., "use /code-atlas best practices"). Mid-prompt tokens are matched against known skill directory names to avoid false positives with file paths like `/src/cli.ts`.
...
**Slash-invocation clearing:** When a **leading** `/skill-name` token matches either a `.claude/commands/<name>.md` file or a known skill directory, the harness expands it inline via `<command-name>` tags instead of routing through the Skill tool. The matcher records the skill as invoked and removes it from pending, since the harness loads it without a Skill tool call. This prevents harness-expanded skills from staying pending forever. Mid-sentence `/skill-name` tokens (e.g., "via /code-atlas best practices") are NOT auto-cleared — the harness only expands leading tokens, so mid-sentence tokens stay pending for Skill tool invocation.

**Keyword enrichment for bare prompts:** When `/skill-name` tokens consume the entire prompt (leaving `promptLower` empty), the matcher enriches `promptLower` with the identified skills' keyword sources before running keyword matching. This ensures companion skills are discovered even for terse commands like `/elegant` or `/simplify`.
...
| `priority` | `number` | n/a | Display priority for ordering when multiple skills match equally |
| `docs` | `string[]` | n/a | URLs to external documentation; informational, not used in scoring |
| `disable-model-invocation` | `boolean` | n/a | When `true`, skill is context-only: matches keywords but loads via Read |

**Top-level fields** (outside `metadata:`) also parsed by the matcher:

| Field | Type | Weight | Purpose |
| --- | --- | --- | --- |
| `validate` | `array` | n/a | Anti-pattern rules checked against file content (see below) |
| `chainTo` | `array` | n/a | Skill chaining rules — after invocation, queue companion skills on pattern match |
| `retrieval` | `object` | n/a | Schema-validated only when the explicit skill validator is requested. Not read by the runtime matcher (verified 2026-04-25 — `grep retrieval .claude/hooks/skill-matcher/skill-matcher.ts` returns empty). Put real discovery signals in `keywords` / `intent-patterns`. |
...
See [frontmatter-reference.md](frontmatter-reference.md) for full field-by-field schema and examples. See [skill-template.md](../templates/skill-template.md) for a copyable YAML template.

---

## Scoring Algorithm

1. **Keyword + intent scoring (weight = 1):** Each keyword/intent-pattern regex is tested against the normalized prompt. Matches preceded by negation words (`not`, `don't`, `without`, `skip`, `ignore`, `except`, `no`) within 30 chars are discarded.
...
4. **Gap analysis with file-trigger bypass:** Explicit (`/skill-name`), use-promoted ("use X and Y"), and file-trigger-matched skills always qualify. Remaining keyword-only skills are included if they score ≥ 50% of the top keyword-only score, capped at 5. Directory-like paths in prompt text (e.g., `apps/portal`) also match `**` file-trigger globs via synthetic-child testing. Only leading `/<skill>` tokens are auto-cleared as harness-expanded; mid-sentence `/<skill>` tokens stay pending for Skill tool invocation.

5. **Confidence tiers** (informational, in output):

| Tier | Condition                | Meaning        |
| ---- | ------------------------ | -------------- |
| HIGH | hits/totalSignals > 30%  | Strong match   |
| MED  | hits/totalSignals > 15%  | Moderate match |
| LOW  | hits/totalSignals <= 15% | Weak match     |

6. **Hard confidence floor:** Selection rejects any skill with `hits / totalSignals < MIN_CONFIDENCE` (10% default, 3% with `metadata.skipMetaAnalysisDiscount`). Inflated keyword/intent-pattern lists silently drop single-hit prompts below the floor — prefer ≤30 totalSignals when single-hit prompts must fire the skill. See `skill-matcher-signal-budget.md` for the authoring contract.

---

## Cache Strategy

**File:** `.claude/state/skill-cache.json`

The cache stores compiled skill data (regex sources/flags, names, signal counts) alongside the newest `SKILL.md` mtime at cache-write time. On each prompt:
...
**Path:** `.claude/state/sessions/$SESSION_ID/invoked-skills`
...
All session-scoped state — `pending-skills`, `invoked-skills`, `tracker.jsonl`, and `agent-unverified` — lives under the same `sessions/$SESSION_ID/` directory and is cleaned up together at `SessionEnd`.
...
**Example:** When `/debugger` is invoked, its body references `@sentry/`, `captureException`, `instrumentation.(ts|js)`, etc. The debugger's chainTo rule for `sentry` (pattern: `search_issues|get_sentry_resource|analyze_issue_with_seer|captureException`) matches `captureException` in the body text, queuing `sentry` as a required companion skill.
...
- [ ] **Has retrieval?** Optional. Schema-validated only; runtime matcher does not score these. Use `keywords` / `intent-patterns` for actual discovery surface.
