...
| `skill-router.mjs` runtime | Skill discovery and gate/inject enforcement via the `.claude/hooks/skill_*.sh` wrappers; custom `validate`/`chainTo`/`retrieval` blocks are not read by the current router |
| `npm run sync:llm` | Generated-mirror sync only (`scripts/skills/sync-llm.mjs`), no frontmatter parsing |

Run `npm run sync:llm` as the closeout for any edit under `.claude/skills/**` so the `.agents/skills/**` and `.codex/skills/**` mirrors stay byte-identical to the canonical source.

## Skill mirror contract

`scripts/skills/sync-llm.mjs` mirrors a fixed set of skills (`code-atlas`, `fastmcp`, `fastmcp-client-cli`, `supaschema`, `ultracite`, `upstream`) into `.agents/skills/**` and `.codex/skills/**`. The mirror is file-scoped: each mirrored skill declares an explicit file list in `MIRRORED_SKILLS`, and only those files are copied. Adding a new file to a mirrored skill (e.g. a new `references/*.md`) requires adding it to that skill's `MIRRORED_SKILLS` entry, or the file silently never reaches the `.agents`/`.codex` mirrors.

After changing a mirrored skill's file set, add the file to `MIRRORED_SKILLS` and re-run `npm run sync:llm`, then confirm the same file now exists under both `.agents/skills/<skill>/` and `.codex/skills/<skill>/`.
