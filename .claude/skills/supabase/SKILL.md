...
- **supaschema schema & migration workflow** → use the `supaschema` skill and Rule 00 **when** doing any schema change or migration in this repo (declarative `schemaPaths` SQL tree, `supaschema diff` → `check` → `verify`). Overrides the generic "Making and Committing Schema Changes" section.
