...
4. Run `npm run format` when formatting or lint-fix remediation is needed; use read-only gates such as `npm run lint`, `npm run py:lint`, and `npm run py:format:check` only to validate.
5. Exercise the changed behavior directly.

Do not stop at step 4. The direct exercise is the whole point. For repo-wide source, import, dependency, consumer, generated-surface, or package-boundary claims, compare the implementation against **Code Atlas** (Rule 10) before reading source broadly — `npm run code-atlas:query -- trace-change <target> --json`, `npm run code-atlas:query -- regression-scope --json`, or the narrower `npm run code-atlas:query -- <kind> <value> --json` (kinds: `route file package symbol db policy api worker search consumers entrypoints impact pre-edit trace-change file-owners regression-scope validate-coverage health mcp-status`; graph cache is `.tmp/code-atlas/atlas.json`, never committed). Then verify the decisive paths directly with `cclsp` (Rule 06) and a source read.
...
- the read-only contract: probe Rule 11's local supaschema server and fixed Code Atlas bridge to confirm no tool mutates repo state
