...
function runHookEventWithState(
  eventName: string,
  payload: Record<string, unknown>,
  stateDir: string
): ReturnType<typeof handleAgentHookEvent> {
  const previous = process.env.SUPASCHEMA_AGENT_HOOK_STATE_DIR;
  process.env.SUPASCHEMA_AGENT_HOOK_STATE_DIR = stateDir;
  try {
    return handleAgentHookEvent(eventName, payload, { runtime: "claude" });
  } finally {
    if (previous === undefined) {
      delete process.env.SUPASCHEMA_AGENT_HOOK_STATE_DIR;
    } else {
      process.env.SUPASCHEMA_AGENT_HOOK_STATE_DIR = previous;
    }
  }
}
...
): Promise<{ env: NodeJS.ProcessEnv; log: string; project: string }> {
...
`
  ...
  return { env: { ...process.env, npm_execpath: npmExecPath }, log, project };
}
...
describe("llm surface sync hook", () => {
  ...
  it("runs sync:llm after direct edits to Claude-owned skill surfaces", async () => {
    const { env, log, project } = await syncHookProject();
  ...
  it("uses Codex Stop JSON output while syncing changed Claude surfaces", async () => {
    const { env: fixtureEnv, log, project } = await syncHookProject();
    const env = { ...fixtureEnv, CODEX_PROJECT_DIR: project };
...
describe.skipIf(!hasPrivateContextHooks)("agent-hooks response-evidence recording", () => {
  ...
  it("records a completed Bash run as evidence so a truthful verification claim is not flagged", async () => {
    ...
    runHookEventWithState(
      "UserPromptSubmit",
      { prompt: "do the task", session_id: session },
      stateDir
    );
    runHookEventWithState(
      "PostToolUse",
    ...
    const stop = runHookEventWithState(
      "Stop",
      {
    ...
    expect(JSON.stringify(stop.output)).not.toContain(claimReason);
  });

  it("flags a domain verification claim that contradicts a recorded failure", async () => {
    const stateDir = await mkdtemp(join(tmpdir(), "supa-evidence-state-"));
    const session = "evidence-contradicted";
    ...
    runHookEventWithState(
      "PostToolUse",
      {
        ...
        tool_name: "Bash",
        tool_response: { interrupted: true, stderr: "tests failed", stdout: "" },
      },
      stateDir
    );
    const stop = runHookEventWithState(
    ...
    expect(JSON.stringify(stop.output)).toContain(claimReason);
  });
  ...
  it("does not turn source reads containing process.exitCode into Stop blockers", async () => {
    ...
    expect(JSON.stringify(stop.output)).not.toContain("sed -n");
    expect(JSON.stringify(stop.output)).not.toContain("verification command failed");
  });
});
