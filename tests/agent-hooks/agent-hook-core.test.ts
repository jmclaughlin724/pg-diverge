...
describe("narrow Bash enforcement", () => {
  ...
  it("blocks parser-confirmed literal PostgreSQL DDL arguments and heredocs", () => {
  ...
  it("blocks only high-confidence dangerous recursive forced deletion", async () => {
    ...
    expect(evaluate('rm -rf / "unterminated', { env, root }).action).toBe("block");
...
describe("skill routing context", () => {
  it("never authorizes tools from pending skill state", async () => {
    ...
    expect(ordinaryEdit.output.hookSpecificOutput?.permissionDecision).toBeUndefined();
  ...
  it("matches curated keywords and file triggers from parsed skill frontmatter", async () => {
    ...
    expect(fileTriggered.output.hookSpecificOutput?.permissionDecision).toBeUndefined();
  ...
  it("carries pending prompt context to subagents without blocking tools", async () => {
    ...
    expect(subagentTool.output.hookSpecificOutput?.additionalContext).toBeUndefined();
...
interface HookFixture {
  fastmcpSkill: string;
  options: { root: string; runtime: "claude" };
  root: string;
  stateDir: string;
}
...
async function hookFixture(): Promise<HookFixture> {
  ...
  return { fastmcpSkill, options: { root, runtime: "claude" }, root, stateDir };
}
