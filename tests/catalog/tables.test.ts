...
describe("catalog table extraction", () => {
  it("models partition children with attach metadata instead of orphaning child indexes", async () => {
          ...
          return Promise.resolve({
            rows: [
              ...
              },
              {
                columns: ["id"],
                contype: "f",
                definition: "FOREIGN KEY (id) REFERENCES app.events(id)",
                function_dependencies: ["app.validate_event"],
                name: "events_2026_01_parent_fkey",
                oid: "2",
    ...
    const primary = objects.find((object) => object.key === "constraint:app.events_pkey:events");
    expect(primary?.metadata).toMatchObject({
      constraintColumns: ["id"],
      constraintType: "CONSTR_PRIMARY",
    });
    const foreign = objects.find(
      (object) => object.key === "constraint:app.events_2026_01_parent_fkey:events_2026_01"
    );
    expect(foreign?.dependencies).toContain("app.events");
    expect(foreign?.dependencies).toContain("app.validate_event");
    expect(foreign?.metadata).toMatchObject({
      constraintColumns: ["id"],
      constraintType: "CONSTR_FOREIGN",
      foreignKeyTarget: { columns: ["id"], schema: "app", table: "events" },
    });
  });
});
