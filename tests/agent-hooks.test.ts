...
const lineageSql =
  "-- Generated by pg-diverge.\n-- pg-diverge: lineage from=abc to=def\nSET lock_timeout = '5s';\n";
...
describe("claude generated-migration edit hook", () => {
  ...
  it("blocks edits to migrations carrying the lineage marker", async () => {
    ...
    expect(result.stderr).toContain("pg-diverge diff");
  });
...
describe("codex generated-migration tool gate", () => {
  const script = ".codex/hooks/pg-diverge-tool-gate.mjs";
  ...
  it("denies apply_patch updates to migrations carrying the lineage marker", async () => {
    ...
    expect(output.hookSpecificOutput?.permissionDecisionReason).toContain("pg-diverge diff");
  });
