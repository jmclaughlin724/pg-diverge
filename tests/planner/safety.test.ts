...
describe("view replace cast compatibility", () => {
  const srcTable = "CREATE TABLE app.src (id uuid, note text);\n";
  const fromView = `${srcTable}CREATE VIEW app.v AS SELECT s.id::uuid AS id, s.note::text AS note FROM app.src s;`;

  it("blocks cast-type changes that CREATE OR REPLACE VIEW cannot apply", async () => {
    const plan = await diff(
      fromView,
      `${srcTable}CREATE VIEW app.v AS SELECT s.id::uuid AS id, s.note::varchar AS note FROM app.src s;`
    );
  ...
  it("keeps cast-stable compatible replacements on the OR REPLACE path", async () => {
    const plan = await diff(
      fromView,
      `${srcTable}CREATE VIEW app.v AS SELECT s.id::uuid AS id, s.note::text AS note FROM app.src s WHERE s.note IS NOT NULL;`
    );
  ...
  it("honors an exact destructive hint as a guarded drop + create request", async () => {
    const plan = await diff(
      fromView,
      `${srcTable}CREATE VIEW app.v AS SELECT s.id::uuid AS id, s.note::text AS note FROM app.src s WHERE s.note IS NOT NULL;`,
      { hints: { destructive: ["view:app.v"] } }
    ...
    const operation = plan.operations.find((item) => item.key === "view:app.v");
    expect(operation?.blocked).toBe(false);
    expect(operation?.metadata.viewDropRequired).toBe(true);
    expect(operation?.metadata.destructiveDisposition).toBe("destructive-hint");
    const sql = renderMigration(plan, { includeHeader: false });
    expect(sql).toContain('DROP VIEW IF EXISTS "app"."v";');
  });
});
