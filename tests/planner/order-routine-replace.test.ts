...
describe("routine drop vs relation replace ordering", () => {
  ...
  it("pre-drops a replaced trigger whose before-state function depends on a replaced relation", async () => {
    const from = `
      ...
      CREATE TABLE app.t1 (id bigint);
      CREATE TABLE app.t2 (id bigint);
      CREATE FUNCTION app.oldf() RETURNS trigger LANGUAGE plpgsql AS $$
      BEGIN
        PERFORM * FROM app.t1 WHERE id = NEW.id;
        RETURN NEW;
      END;
      $$;
      CREATE FUNCTION app.newf() RETURNS trigger LANGUAGE plpgsql AS $$
      BEGIN
        RETURN NEW;
      END;
      $$;
      CREATE TRIGGER trg AFTER INSERT ON app.t2 FOR EACH ROW EXECUTE FUNCTION app.oldf();
    `;
    const to = `
      CREATE SCHEMA app;
      CREATE UNLOGGED TABLE app.t1 (id bigint);
      ...
      END;
      $$;
      CREATE TRIGGER trg AFTER INSERT ON app.t2 FOR EACH ROW EXECUTE FUNCTION app.newf();
    `;
    ...
    expect(plan.diagnostics.filter((item) => item.severity === "error")).toEqual([]);
    const keys = plan.operations.map((operation) => operation.key);
    expect(keys.indexOf("pre-drop:trigger:app.trg:t2")).toBeGreaterThanOrEqual(0);
    expect(keys.indexOf("pre-drop:trigger:app.trg:t2")).toBeLessThan(
      keys.indexOf("pre-drop:function:app.oldf()")
    );
    expect(keys.indexOf("pre-drop:function:app.oldf()")).toBeLessThan(keys.indexOf("table:app.t1"));

    const sql = renderMigration(plan, { includeHeader: false });
    const dropTrigger = sql.indexOf('DROP TRIGGER IF EXISTS "trg" ON "app"."t2";');
    const dropOldFunction = sql.indexOf('DROP FUNCTION IF EXISTS "app"."oldf"();');
    expect(dropTrigger).toBeGreaterThanOrEqual(0);
    expect(dropOldFunction).toBeGreaterThan(dropTrigger);
  });
});
