...
describe("config DX", () => {
  ...
  it("does not rewrite existing environment and sync ownership during init merge", () => {
    ...
    expect(merged.sync).toEqual(
      expect.objectContaining({ targets: expect.objectContaining({ local: expect.any(Object) }) })
    );
    ...
    expect(supabaseMerged.environments).toEqual({
      local: { databaseUrl: oldLocalUrl },
      production: { databaseUrl: oldProductionUrl },
    });
    expect(supabaseMerged.sync).toEqual(
      expect.objectContaining({ targets: expect.objectContaining({ local: expect.any(Object) }) })
    );
  });
  ...
  it("validates source defaults and redacts inline database URLs", async () => {
    ...
    expect(diagnostics.some((item) => item.field === "sources.to")).toBe(false);
    const text = JSON.stringify(diagnostics);
    expect(text).toContain("postgresql://postgres:[redacted]@example.com/app");
    expect(text).not.toContain("secret");
    expect(() => resolveConfig({ sources: { from: "auto", to: "dir:legacy" } })).toThrow();
  });
...
describe("raw CLI errors", () => {
  it("keeps migrations replay scoped to types instead of drift commands", () => {
    ...
    const types = spawnSync(
      process.execPath,
      [cliPath, "types", "--from", source, "--out", "stdout"],
      {
        cwd,
        encoding: "utf8",
      }
    );
    const removedSourceAlias = spawnSync(
      process.execPath,
