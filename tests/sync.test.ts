...
describe("sync (no target)", () => {
  it("dry-runs pending files after the replay-safety gate", async () => {
    const root = await mkdtemp(join(tmpdir(), "pgd-sync-"));
    await writeFile(
  ...
  it("refuses when a pending migration fails the replay-safety check", async () => {
    const root = await mkdtemp(join(tmpdir(), "pgd-sync-bad-"));
...
describe.skipIf(!databaseUrl)("sync (against a target)", () => {
  it("refuses on ghost history", async () => {
    ...
    await admin.connect();
    const db = `pgd_sync_${process.pid}_${Math.random().toString(16).slice(2, 8)}`;
    ...
    try {
      ...
      await target.end();
      const root = await mkdtemp(join(tmpdir(), "pgd-sync-ghost-"));
      await writeFile(
