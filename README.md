...
**From declarative schema to fully synced database in milliseconds with one command.** No ORM, no Docker, and no shadow database needed. pg-diverge diffs the schema your SQL files declare against the schema your PostgreSQL or Supabase database actually has, and writes a migration that is safe to run twice.
