...
That single command does the whole job: it diffs your applied state (the resolved database, falling back to `git:HEAD`) against your schema tree (`supabase/schemas` by default) and writes the migration file into `supabase/migrations`. Every default is printed to stderr and overridable (`--from`, `--to`, `--name`, `--out stdout`):
...
The loop — every command defaults to the applied state (database, then `git:HEAD`), your schema tree, and your migrations directory, so flags are for overrides, not for daily use:
...
npx pg-diverge check
npx pg-diverge verify
...
npx pg-diverge diff --fail-on-diff --quiet

# Migration gate: prove the PR's migration applies twice and lands on target
npx pg-diverge verify --from "git:origin/$BASE_REF"
```

## Commands

Zero-flag defaults across the workflow: `--from` resolves to the database (then `git:HEAD`), `--to` to the config schema tree, migrations land in `config.migrationsDir` with a derived name, `check` gates the whole migrations directory, and `verify` proves the newest pending migration. Every applied default is printed to stderr.
...
| `verify` | Apply-twice + catalog-parity + reconvergence proof for the newest pending migration in disposable databases (`--keep-databases` for debugging) |
