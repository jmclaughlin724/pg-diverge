...
if (!databaseUrl) {
  fail(
    `${usage}\nno database URL: pass --database-url, set PG_DIVERGE_DATABASE_URL, or run near supabase/config.toml`,
  );
}
...
const changeSet = `
CREATE TABLE IF NOT EXISTS public.pg_diverge_benchmark_probe (
  id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  label text NOT NULL,
  recorded_at timestamptz NOT NULL DEFAULT now()
);
CREATE INDEX IF NOT EXISTS pg_diverge_benchmark_probe_label_idx
  ON public.pg_diverge_benchmark_probe (label);
COMMENT ON TABLE public.pg_diverge_benchmark_probe IS 'pg-diverge benchmark change-set probe';
CREATE OR REPLACE VIEW public.pg_diverge_benchmark_probe_view AS
  SELECT id, label FROM public.pg_diverge_benchmark_probe WHERE id > 0;
`;
...
// Scope the fixture to pg-diverge's supported DDL surface so every engine in
// the comparison diffs the same schema. Statements pg-diverge fails closed on
...
for (const statement of firstPass.ordered) {
  ...
  if (extraction.diagnostics.some((item) => item.severity === "error")) {
    dropped.push({ error: "pg-diverge support-matrix exclusion", sql: statement });
    excluded += 1;
    continue;
  }
...
writeFileSync(
  ...
  `${JSON.stringify({ pgDivergeAdapter: "postgres", schemas: treeSchemas(fromSql) }, null, 2)}\n`,
);
writeFileSync(
