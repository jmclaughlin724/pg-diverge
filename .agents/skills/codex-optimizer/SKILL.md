...
1. Read the live repo files first: root `AGENTS.md`, `.claude/rules/12-skill-loading-enforcement.md`, `.claude/rules/01-operating-rules.md`, and the touched sync owner `scripts/skills/sync-llm.mjs`.
2. For OpenAI or Codex product/API facts, use the official OpenAI docs. Local sync scripts prove repo topology only.
3. Treat `.claude/**` as canonical for rules, skills, and Claude hooks. Treat `.codex/config.toml`, `.codex/hooks.json`, and `.codex/hooks/**` as manual Codex runtime surfaces. Treat `.codex/rules/**` and `.agents/skills/**` as generated or compatibility targets unless a rule names the file as repo-owned.
...
- `.codex/rules/**` are generated Codex `.rules` mirrors of `.claude/rules/**`; Claude Markdown policy is rendered as comments so the files remain valid Codex command-rule files. `.agents/skills/**` is the full directory mirror of `.claude/skills/**`. `.codex/hooks/**` is a full directory mirror of `.claude/hooks/**`; hook sources must be runtime-aware when Claude and Codex output contracts differ. `.codex/agents/**` is generated from `.claude/agents/**` as Codex TOML.
- Codex runtime registration and generated mirrors are repo-local unless Rule 13 explicitly promotes a surface into the downloadable consumer package or scaffold. Rule 13 promotes only the consumer-safe `.codex/hooks.json`, which must wire the supaschema generated-migration block, schema auto-diff, and LLM surface sync hooks. Do not add `.codex/hooks/context-*`, `.codex/agents/**`, optimizer skills, internal rules, or `scripts/agent-hooks/**` to `package.json` `files` or `bin/scaffold.mjs` just because they are valid repo-local Codex surfaces.
...
- `.mcp.json` owns the repo MCP registry. `.codex/config.toml` MCP entries are reconciled by the catalog/agent guards; do not duplicate or retarget registry entries without updating the guard-owned source of truth.
- `npm run sync:llm` maps Claude-owned skills, hooks, agents, and rules into Codex and `.agents` targets. Codex hook registration still lives in `.codex/hooks.json`, but the scripts under `.codex/hooks/**` are generated from `.claude/hooks/**`.
...
4. `npm run sync:llm` mirrors the full `.claude/skills/**` tree into `.agents/skills/**`, mirrors the full `.claude/hooks/**` tree into `.codex/hooks/**`, renders `.claude/agents/**/*.md` into `.codex/agents/**/*.toml`, and renders `.claude/rules/**/*.md` into `.codex/rules/**/*.rules`. If a generated target exists without the matching `.claude/**` owner, classify it as generated-mirror drift. Promote the source into `.claude/**` only if it should stay.
...
- `.claude/skills/**`, `.claude/hooks/**`, `.claude/agents/**`, or `.claude/rules/**`: `npm run sync:llm` to refresh generated targets, then `npm run guard:agent` to verify agent-surface parity.
- `.claude/rules/**` or root `AGENTS.md`: `npm run guard` (`scripts/guards/check-all.mjs`) for the full agent-surface gate.
- `.codex/config.toml`, `.codex/rules/**`, or `.mcp.json`: `npm run guard:agent`, then `npm run sync:llm` if a mirrored skill changed.
