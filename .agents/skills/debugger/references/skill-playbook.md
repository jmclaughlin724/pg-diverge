...
2. **Capture the full error surface** with the real gates:
   ...
   - `npm run lint` / `npm run lint:ci` to capture lint/format diagnostics; use `npm run format` for remediation and chain to the `ultracite` skill for rule specifics.
