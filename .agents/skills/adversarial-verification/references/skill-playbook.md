...
Do not stop at step 4. The direct exercise is the whole point. For repo-wide source, import, dependency, consumer, generated-surface, or package-boundary claims, compare the implementation against **Code Atlas** (Rule 10) before reading source broadly — `npm run code-atlas:query -- trace-change <target> --json` or the narrower `npm run code-atlas:query -- <kind> <value> --json` (kinds: `route file package symbol db policy api worker search consumers entrypoints impact pre-edit trace-change file-owners validate-coverage health mcp-status`; graph cache is `.tmp/code-atlas/atlas.json`, never committed). Then verify the decisive paths directly with `cclsp` (Rule 06) and a source read.
