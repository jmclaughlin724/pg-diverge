...
export function renderMigrationSplit(
  ...
  const concurrentSql = `-- pg-diverge: run each statement outside a transaction block.\n-- CREATE INDEX CONCURRENTLY cannot run inside a transaction.\n${concurrentBody}\n`;
...
function renderHeader(plan: MigrationPlan, config: PgDivergeConfig, version?: string): string {
  ...
  const transaction = hasConcurrentIndex
    ? "-- pg-diverge: transaction=false required for concurrent index statements\n"
    : "";
  const generator = version ? `pg-diverge ${version}` : "pg-diverge";
...
function spliceGuard(object: SchemaObject): string {
  ...
  if (!facts || typeof facts !== "object") {
    throw new Error(
      `object ${object.key} has no render guard facts; re-extract the source model with this pg-diverge version`,
    );
  }
  ...
  if (typeof offset !== "number" || offset < 0 || offset > object.sql.length) {
    throw new Error(
      `object ${object.key} has no render guard offset; re-extract the source model with this pg-diverge version`,
    );
  }
