...
- `replay.ts` reconstructs a source model from `migrations:` histories.
...
- `migrations:` is a runtime source kind for `types --from` and for the `diff`/`plan`/`sync` before-state only when it resolves to the configured `migrationsDir`. Keep it out of generation targets, `verify`, and drift lanes.
