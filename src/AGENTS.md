...
- `catalog/query.ts`, `catalog/extract.ts`, `catalog/types.ts`, `catalog/tables.ts`, `catalog/sequences.ts`, `catalog/foreign.ts`, `catalog/grants.ts`, `catalog/comments.ts`, `catalog/selfcheck.ts` — read-only catalog extraction, live database model construction, and parity checks
...
- `verify/migration.ts`, `check/migration.ts`, `check/report.ts`, `diagnostics/diagnostics.ts`, `diagnostics/catalog.ts` — replay-safety verification, migration SQL checks, and diagnostic construction
- `scan/rules.ts`, `scan/model.ts`, `scan/aggregate.ts`, `scan/onboard.ts` — rule packs, safety scoring, JSON report validation, aggregation, badge rendering, and onboarding readiness
- `pipeline/diff.ts`, `pipeline/deploy-safety.ts`, `pipeline/type-safety.ts` — source-to-plan construction and workflow deploy gates
- `contract/schema.ts`, `contract/type-diff.ts`, `contract/intake.ts` — schema contract shape, type-contract drift, and intake validation
...
- `benchmark/run.ts`, `benchmark/diff-score.ts`, `benchmark/fixtures.ts`, `verify/corpus.ts` — benchmark and corpus verification surfaces
- `audit.ts`, `doctor.ts`, `hash.ts`, `paths.ts`, `redaction.ts`, `validators.ts`, `types.ts` — focused single-concern flat roots and shared type catalog
