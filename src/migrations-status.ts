...
export interface MigrationsStatusReport {
  ...
  outOfOrder: string[];
  /** Pending files generated by pg-diverge, with their lineage markers. */
  ...
  if (!options.databaseUrl) {
    ...
    diagnostics.push(
      diagnostic(
        ...
        { hint: "Pass --database-url or set PG_DIVERGE_DATABASE_URL to compare against a target." },
      ),
    );
...
export function renderMigrationsStatus(report: MigrationsStatusReport): string {
  ...
  for (const file of report.pending) {
    ...
    lines.push(`  pending: ${file}${lineage ? " (pg-diverge lineage)" : ""}`);
  }
...
async function readHistory(
  ...
  try {
    ...
    diagnostics.push(
      diagnostic(
        "PD_MIGRATIONS_TARGET_UNAVAILABLE",
        "error",
