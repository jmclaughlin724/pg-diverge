...
import { type BuildInfo, isUnreleasedBuild, readBuildInfo } from "../build-info.js";
...
function withGeneratedHeader(contents: string, info: BuildInfo, fingerprint: string): string {
  const build = isUnreleasedBuild(info)
    ? `${info.version} (${info.commit === null ? "unknown commit" : info.commit.slice(0, 12)}, unreleased build)`
    : info.version;
