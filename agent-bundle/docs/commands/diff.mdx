...
| ------------------ | -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--from`           | source         | Before-state source. Defaults to `config.sources.from`. A `migrations:` value must match the selected migrations directory.                                                                                                                                                                                 |
| `--to`             | source         | Desired-state source. Defaults to `dir:<config.schemaPaths[0]>`. Migration replay is not supported here.                                                                                                                                                                                                    |
| `--out`            | path \| stdout | Write SQL to a specific file or print to stdout.                                                                                                                                                                                                                                                            |
| `--name`           | snake_case     | Override the descriptive part of the timestamped migration filename.                                                                                                                                                                                                                                        |
| `--migrations-dir` | path           | Override the directory where timestamped migrations are written.                                                                                                                                                                                                                                            |
| `--replace`        | path           | Replace one unapplied supaschema-generated migration in the selected migrations directory. The file must contain lineage metadata, the planned `--from` fingerprint must match the file's original lineage baseline, and any resolvable configured target must not record the migration version as applied. |
| `--schema`         | names          | Diff only the comma-separated PostgreSQL schema names.                                                                                                                                                                                                                                                      |
| `--dry-run`        | boolean        | Print the migration and target path without writing files.                                                                                                                                                                                                                                                  |
| `--json`           | boolean        | Print a structured payload with fingerprint, operations, and SQL.                                                                                                                                                                                                                                           |
| `--fail-on-diff`   | boolean        | Exit `3` when the plan contains operations. Use this for drift gates.                                                                                                                                                                                                                                       |
| `--no-check-chain` | boolean        | Skip the lineage-chain gate against pending supaschema migrations.                                                                                                                                                                                                                                          |
| `--summary`        | boolean        | Print operation and diagnostic counts before any error exit.                                                                                                                                                                                                                                                |
| `--write-hints`    | path           | Write blocked destructive object keys as a `hints.destructive` skeleton.                                                                                                                                                                                                                                    |
| `--timing`         | boolean        | Print extract, plan, and render timings to stderr.                                                                                                                                                                                                                                                          |
| `--watch`          | boolean        | Re-run on `dir:` source changes. Implies `--dry-run --summary`.                                                                                                                                                                                                                                             |
...
Before planning a `git:` to `dir:` diff, supaschema checks the workspace for already-open migration units. Dirty generated TypeScript/Zod outputs block with `SUPA_DIFF_GENERATED_CONTRACT_DIRTY`, dirty migration files block with `SUPA_DIFF_MIGRATIONS_DIRTY`, and scoped `--schema` diffs also block on dirty global config (`SUPA_DIFF_CONFIG_DIRTY`) or dirty schema files outside the requested filter (`SUPA_DIFF_SCOPED_DIRTY_SCHEMA`). Close that unit first; do not add hints or change the baseline to bypass it. A `git:` baseline blocks on any dirty migration file because the baseline cannot see uncommitted files; a `migrations:` replay baseline reads those same files as its before-state, so only lineage-bearing generated files must be closed or pruned first — a hand-authored tail is legitimate replay input for an unscoped diff, though a scoped diff cannot prove the tail's schema scope.
