...
| `SUPA_SOURCE_MIGRATIONS_TYPEGEN_ONLY` | error | Generation was given a `migrations:` replay source. Use it only with `supaschema types --from`; use `git:`, `dir:`, `dump:`, `catalog:`, or `empty:` for generation. |
