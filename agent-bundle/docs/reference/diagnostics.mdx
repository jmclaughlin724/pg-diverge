...
| `SUPA_CHECK_SECURITY_DEFINER_SEARCH_PATH` | warning | A `SECURITY DEFINER` function lacks an empty function-local `SET search_path = ''`. |
...
| `SUPA_RULE_SECDEF_SEARCH_PATH` | warning | A `SECURITY DEFINER` routine does not pin `search_path = ''`, so a caller can shadow an unqualified reference and run code as the routine owner. |
