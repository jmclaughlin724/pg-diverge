...
| Hook | Event | Purpose |
| --- | --- | --- |
| `supaschema hook generated-artifact-edit` | `PreToolUse` | Blocks direct edit-tool writes to lineage-marked migration SQL and the exact configured `typesFile` / `zodFile` outputs. |
...
The registered artifact hook receives only actual edit tools: `Write|Edit|MultiEdit|apply_patch` for Claude and `apply_patch` for Codex. It is not a shell-policy hook or a filesystem boundary. Only an `apply_patch` delete-only change for a stale generated migration is the reviewed recovery lane; deletion of configured generated contracts stays blocked. If malformed input or invalid config prevents classification, the hook denies with `SUPA_GENERATED_ARTIFACT_GUARD_FAILED`; run `supaschema config validate`, repair the first error, run `supaschema doctor`, and retry. When a generated artifact is targeted, change declarative SQL or generator config, run `supaschema doctor`, then run `supaschema sync`; use `supaschema types` only when schema sources already match the intended state. Preserve unexplained generated drift, review the regenerated diff, and use `supaschema explain <CODE>` for the structured procedure.

The bundle does not publish or register this repository's parser-backed secret, literal-DDL, or broad-deletion Bash guard. That narrow guard is maintainer tooling for developing Supaschema itself.
...
If config or approval does not permit automatic sync, the hook stays on the diff/check lane. If `check` or `sync` fails, the hook emits continuation feedback so Claude or Codex investigates the root source, searches for correlated migration failures, fixes the canonical owner, and reruns the failing command.
