...
<CardGroup cols={2}>
  <Card title="Imperative (traditional)" icon="list-ordered">
...
<Frame>
  <img
    src="/images/concepts/legacy-flow.svg"
    alt="Traditional declarative workflow using a shadow database, schema replay, diff, apply, and type introspection"
  />
</Frame>
...
<Frame>
  <img
    ...
    alt="supaschema workflow using embedded PostgreSQL parsing, AST comparison, guarded migration rendering, and offline type generation"
  />
</Frame>
...
- Your schema files should express only the objects _you_ own.
...
:::note
You can still _reference_ managed schema objects (foreign keys to `auth.users`, calls to `auth.uid()` in policies, etc.). supaschema only prevents you from declaring _ownership_ of objects inside those schemas.
:::
