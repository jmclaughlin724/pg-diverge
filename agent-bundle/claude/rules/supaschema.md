...
- The implementation lane for diff/plan/sync is: resolve configured `schemaPaths`, `sources`, and `migrationsDir`; extract parser-backed `from` and `to` schema models; extract a structured migration corpus from existing migrations; pass both schema models and corpus into planning/rendering; then check, verify, and reconcile against migration history. Existing migrations cannot be used only for lineage/history while the planner ignores their contents.
...
- Run `supaschema check` on every generated or hand-authored migration. Run `supaschema verify` only as an explicit runtime-proof lane when a disposable database is intentionally available; it is not part of generation, typegen, or automatic sync. Keep `transactionMode: "per-migration"` for transactional runners such as `supabase db push`.
