...
- Consumer setup is package install plus one explicit setup command with the consuming project's package manager. Default `supaschema init` writes config, configured schema directories, configured migration directories, six canonical package scripts when `package.json` exists, and pending path-confirmation state. Those scripts are `supaschema:sync` for the full `supaschema sync` workflow, plus explicit lanes: `supaschema:diff` for `supaschema diff`, `supaschema:stage` for `supaschema stage`, `supaschema:apply` for `supaschema apply`, `supaschema:types` for `supaschema types`, and `supaschema:check` for `supaschema check`; setup diagnostics and database execution verification stay on direct CLI commands (`supaschema config validate` and `supaschema verify`). It does not write active AI-agent rules, hooks, skills, settings, `AGENTS.md`, `CLAUDE.md`, or backup directories, and it leaves existing active agent surfaces untouched. The raw AI-agent bundle ships separately under `node_modules/supaschema/agent-bundle/`; install enforcement surfaces only by reviewing `agent-bundle/INSTALL.md` and manually applying its copy/merge instructions after user approval. Maintainer-only Claude/Codex optimization and context-enforcement tooling remains repo-local under Rule 13. `supaschema init` is idempotent: existing JSON config files and existing `supaschema:*` package scripts are left untouched unless explicit repair is requested, and JavaScript config files are not loaded or converted.
