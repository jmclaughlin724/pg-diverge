...
- **Schema tree:** `schemaPaths`, `sources.to`, and `migrationsDir`. Edit only the configured schema roots; `dir:` sources read nested `.sql` files recursively and merge them into one model. `sources.to` is the explicit target for zero-source-flag `diff`, `plan`, and `verify`. Generated migrations write to `migrationsDir`; existing generated lineage there proves the migration-tree baseline.
- **Diff baseline:** `sources.from` and `sources.to`. Install writes `sources.from: "auto"` and `sources.to: "dir:<schemaPaths[0]>"`; for generation, `auto` resolves `git:HEAD` only as a candidate source snapshot and must match generated migration lineage when migrations exist. Generation examples or fixture projects can pin source-backed `dump:`, `dir:`, `git:`, `catalog:`, or `empty:` values when that source is the project contract. Use `database:` only in explicit inspect, verify, drift, selfcheck, target-safety, or catalog-snapshot workflows.
- **Generated contracts:** `typesFile`, `zodFile`, `workflow.type_generation`, `workflow.zod_generation`, and `workflow.type_usage`. `supaschema types` creates or refreshes TypeScript and Zod outputs from the configured schema source, including schema-defined views, materialized views, view-on-view dependencies, functions, enums, and composites; `zod_validated` means agents should use generated Zod validators at runtime boundaries. If a view depends on an extension-owned relation outside the configured schema source, fix the supported source/model instead of patching application consumers with casts, aliases, or local contract copies. If a schema-backed generated field remains `unknown`, treat it as missing typegen source/model coverage and fix that owner; do not commit app-side casts, copied contracts, or local DTOs to hide it.
...
2. **Generate the migration:**
   ...
   Zero-source-flag defaults come from `config.sources` and are printed to stderr. For generation, `sources.from: "auto"` resolves to valid `git:HEAD` only as a candidate source snapshot, then `empty:` only for a first migration with no existing migration corpus; existing migrations without a Git baseline produce `SUPA_SOURCE_BASELINE_REQUIRED`, and generated migration lineage mismatches produce `SUPA_MIGRATION_BASELINE_MISMATCH` / `SUPA_MIGRATION_BASELINE_UNSUPPORTED`. `sources.to` points at the configured declarative tree. The file lands in `config.migrationsDir` as `<UTC timestamp>_<derived name>.sql`. Pass `--name <snake_case>` only when the human wants a specific file name. The write is no-clobber and chain-gated; named/file-output empty plans fail with `SUPA_DIFF_EMPTY_PLAN`. If it exits 2, read the diagnostic:
   ...
   - `SUPA_ROUTINE_DYNAMIC_SQL_DEPENDENCY_HINT_REQUIRED` / `SUPA_PLAN_COLUMN_DEPENDENT_REWRITE_REQUIRED` — do not guess around routine bodies or column dependents. Add exact relation/column dependencies to `hints.routineDependencies`, rewrite the dependent routine/view/policy/trigger so it no longer references the changed column, split the migration, or use a reviewed explicit migration.
