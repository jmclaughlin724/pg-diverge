...
The installed package also contains an offline, byte-exact copy of every public MDX page under `node_modules/supaschema/agent-bundle/docs/`. Start with `agent-bundle/docs/index.md` when hosted documentation is unavailable. These reference files stay inside `node_modules`; `supaschema init` does not copy them into the consumer project.
...
- Sources for either side of a diff: `dir:<tree>`, `git:<ref>`, `database:<url|$ENV>`, `dump:<file.sql>`, `catalog:<snapshot.json>`, and `empty:`. `migrations:<migrationsDir>` is allowed only on the before side.
