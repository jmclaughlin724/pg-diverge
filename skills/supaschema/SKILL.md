...
5. **Refresh contracts:** `supaschema types` regenerates TypeScript and Zod from the configured schema source, including views, materialized views, view-on-view dependencies, functions, enums, and composites. Never wait for a deploy or run introspection-based typegen to get correct types.
